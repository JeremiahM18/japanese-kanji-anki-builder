...
Current N5 source-list routing status: N5 enhancement candidates are exhausted for current-level promotion, while known-JLPT rows with no N5 anchor are tracked separately at `20/151` cross-level routing rows triaged (`19` move candidates, `1` rejected duplicate source row, `131` advisory rows still untriaged). These routing rows do not reopen N5 expansion and do not move cards by themselves.
