...
- The current N5 reading audit now credits explicit support coverage from date, counter, and calendar words like `七日`, `十日`, `五分`, `十分`, `四月`, and `五月`, bringing the tracked N5 reading picture to `186/344` covered readings, with `182` covered by JLPT-core rows, `4` by support rows, `142` distinct missing targets, and `16` lower-priority variant gaps still queued.
- `npm run deck:words:ready -- --levels=5` now writes an explicit completion block into `out/word-build/build-summary.json` and the console report so we can see tracked N5 governance coverage, explicit reading-coverage contract coverage, canonical inventory counts, and the live reading-audit summary (`186/344`, `54.1%` covered on the current N5 baseline) before claiming the word deck is complete.
