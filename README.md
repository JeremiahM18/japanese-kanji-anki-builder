...
| N4 word | Golden review partial at `396/457`; `ready_with_deferred_variants` when built with N5 as the selected word-product scope; audio, pitch accent, card-back fields, example reading alignment, and looping animations are complete for shipped rows; platinum not started |
...
- Current N5+N4 word rows: `805`
- N5 word reading coverage: `83.7% (288/344)` when built alone
- N4 word reading coverage: `75.6% (492/651)` when built with N5 as the selected word-product scope
