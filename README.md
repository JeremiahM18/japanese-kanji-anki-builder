...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `102` reviewed assignments from the pinned ignored worksheet; `2110` worksheet rows remain pending |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `203` high-confidence rows, `20` standard-confidence rows, `2` disputed rows, `1987` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1913` insufficient independent source rows, `1850` insufficient independent evidence-lineage rows, `1858` missing Japanese-published source rows, `2` disputed consensus rows, and `87` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `2025` rows needing governed source review: `1858` missing Japanese-published source, `73` weak evidence, `72` contract/consensus mismatch, `20` insufficient independent sources, and `2` disputed consensus, with source-input progress at `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved) and `102` Sou Matome reviewed rows (`102` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
