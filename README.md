...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `413` reviewed assignments and `445` non-importing `source_access_gap` rows from the pinned ignored worksheet; `1354` worksheet rows remain pending |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `365` high-confidence rows, `48` standard-confidence rows, `16` disputed rows, `1783` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1701` insufficient independent source rows, `1430` insufficient independent evidence-lineage rows, `1437` missing Japanese-published source rows, `16` disputed consensus rows, and `190` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1833` rows needing governed source review: `1437` missing Japanese-published source, `179` contract/consensus mismatch, `120` insufficient independent sources, `81` weak evidence, and `16` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved), and `413` Sou Matome reviewed rows plus `445` `source_access_gap` rows (`858` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
