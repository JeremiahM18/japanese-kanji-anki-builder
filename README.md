...
| Surface | Current state | Main gates |
| --- | --- | --- |
| N5 word | `588/588` current word Obsidian v2.5-certified under the strict sentence-audio standard. N5 Gold, Sapphire, and current-standard Platinum are complete at `588/588`; no N5 rows remain in the current v2.5 Obsidian backlog. Legacy word Obsidian proof history remains audit-visible, with older same-target proof events superseded by current v2.5 proof where applicable; it is not current certification. No N5 source-only phrase exclusions remain in the tracked word contract. Live readiness is `ready_with_deferred_variants`; reading coverage is `71.2%` (`245/344`). Release artifact QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:obsidian:certify-status -- --levels=5`, `deck:words:ready -- --levels=5` |
| N4 word | `0/1034` current word Obsidian v2.5-certified. The `700` legacy N4 Obsidian proof targets are legacy history, not current v2.5 certification. The `700` Platinum-passing N4 rows need Obsidian v2.5 rereview; the 334 current N4 word v2 common-word Silver additions are generated-ready with word audio, pitch, required back-side fields, examples, reading breakdowns, support labels, and readiness, but they have not entered Gold/Sapphire/Platinum/Obsidian. Live readiness is `ready_with_deferred_variants`; reading coverage is `77.0%` (`581/755`). Release artifact QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:obsidian:certify-status -- --levels=4`, `deck:words:ready -- --levels=5,4` |
...
Current word Obsidian v2.5 proof covers `588/1622` current N5/N4 generated word rows: N5 is `588/588`, and N4 is `0/1034`. Legacy word Obsidian proof history remains audit-visible at `1118` N5/N4 targets and `1706` raw ledger events; `418` legacy events are superseded by current v2.5 proof. Legacy history does not count as current certification. Native Sapphire structural coverage and current-standard Platinum card-surface inspection are lower-lane prerequisites only; the remaining rows must not be described as current Obsidian v2.5-certified.
...
`deck:words:platinum:source-posture -- --levels=5,4` currently inspects `1288` structurally current-standard entries: `129/1288` have independent source families proven, `1159/1288` are single-source-family, and `0/1288` are missing governed source evidence.
...
- `deck:words:obsidian:rereview-status -- --levels=5,4` uses generated word rows as the denominator and defaults to canonical JSONL proof for migrated N5/N4 word ledgers. It reports current word Obsidian v2.5 proof only when the card has `word-obsidian-v2.5-sentence-audio`, exact identity binding, real sentence-quality review, and exact example-sentence audio proof. Current N5/N4 v2.5 certification is `588/1622`; the older `1118` target proof history is legacy audit history.
- `deck:words:obsidian:certify-status -- --levels=5,4` is the fail-closed word Obsidian gate. It is expected to fail for the current full N5/N4 generated denominator until the remaining `700` N4 Platinum-passing rows complete Obsidian v2.5 proof, and the 334 N4 Silver rows complete Gold, Sapphire, Platinum, and Obsidian.
...
- Phrase-like source rows are rejected from default word inventory unless explicitly curated as dictionary-backed lexical identities; no N5 source-only phrase-exclusion bucket is counted.
