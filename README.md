...
| N4 word | Expanded to `667` governed rows; current word-level placement audit and golden review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Current-standard word platinum has started at `24/667` under `word-platinum-v2-limitation-aware`; full N4 word platinum is not complete. |
...
- N5 word golden review passes against the current `287` generated rows, and the default current-standard word platinum command passes at `287/287`. N4 word golden review passes at `667/667` generated rows; N4 current-standard word platinum is partial at `24/667`, with `643` generated rows still missing current-standard platinum review. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
