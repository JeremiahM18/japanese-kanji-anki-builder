...
That command refreshes generated kanji TSVs, creates kanji-card tokenization artifacts, converts tokenizer/card-reading signals into kanji-scoped review packets, validates the artifacts, and runs the NLP governance gate. Normal kanji tokenizer/primary-reading differences are review context, not automatic defects. Bare-kanji unknown tokens and missing tokenizer readings are counted as tokenizer coverage gaps, not card-risk attention by themselves; unrelated artifact warnings and other hard signals still require attention. It does not run word expansion, word reading-gap discovery, word example reranking, or word sense-fit audits.
...
| Surface | Current state | Main gates |
| --- | --- | --- |
| N5 word | `287` canonical rows plus `20` source-only phrase exclusions. Gold, readiness, tracked-source artifact, Platinum, and strict word Obsidian proof now pass at `287/287`; manual import QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:platinum:n5`, `deck:words:platinum:rereview-status -- --levels=5,4`, `deck:words:platinum:certify-status -- --levels=5` |
| N4 word | `700` canonical rows after finishing the routed N5-source target-N4 move workload. The generated card surface builds at `700/700` with word audio, pitch, required back-side fields, word-level placement, examples, and labels complete; live readiness is `ready_with_deferred_variants` with reading coverage `76.7%` (`579/755`) and no active review-needed/actionable reading backlog. Gold and Platinum now cover `700/700`; stricter Obsidian enforcement currently certifies `112/700`, so `588` N4 word rows still need square-zero substantive rereview before release-ready claims. | `deck:words:ready -- --levels=5,4`, `deck:words:completion:n4`, `deck:words:review:n4`, `deck:words:platinum:n4`, `deck:words:platinum:certify-status -- --levels=4`, `deck:words:expansion-signals -- --levels=5,4` |
| N3 word | `269` canonical rows. Silver generated surface now builds at `269/269` with required back-side fields `2959/2959`, word audio `269/269`, Kanjium pitch `269/269`, reading breakdowns, and deck-policy checks passing for those rows; readiness is still `incomplete`, cumulative reading coverage is `235/1232` (`19.1%`), the Tanos N3 vocabulary lane has `359` review candidates (`350` untriaged, `8` deferred, `1` rejected), and Gold/Platinum/Obsidian are not started. | `deck:words:ready -- --levels=3`, `deck:words:completion:n3`, `deck:words:reading-audit:n3`, `deck:words:triage:n3`, `deck:words:gap-plan:n3`, `deck:words:expansion-candidates:n3` |
| N2 word | `28` canonical rows. Silver generated surface now builds at `28/28` with required back-side fields `308/308`, word audio `28/28`, Kanjium pitch `28/28`, reading breakdowns, and deck-policy checks passing for those rows; readiness is still `incomplete`, cumulative reading coverage is `49/1061` (`4.6%`), the Tanos N2 vocabulary lane has `415` remaining untriaged review candidates, and Gold/Platinum/Obsidian are not started. | `deck:words:ready -- --levels=2`, `deck:words:completion:n2`, `deck:words:reading-audit:n2`, `deck:words:triage:n2`, `deck:words:gap-plan:n2`, `deck:words:expansion-candidates:n2` |
| N1 word | `26` canonical rows. Silver generated surface now builds at `26/26` with required back-side fields `286/286`, word audio `26/26`, Kanjium pitch `26/26`, reading breakdowns, and deck-policy checks passing for those rows; readiness is still `incomplete`, cumulative reading coverage is `41/3284` (`1.2%`), the Tanos N1 vocabulary lane has `1008` remaining untriaged review candidates, and Gold/Platinum/Obsidian are not started. | `deck:words:ready -- --levels=1`, `deck:words:completion:n1`, `deck:words:reading-audit:n1`, `deck:words:triage:n1`, `deck:words:gap-plan:n1`, `deck:words:expansion-candidates:n1` |

Word Platinum uses `word-platinum-v3-evidence-lanes`. Current N5/N4 generated word denominator is `987` rows. Structural Platinum is `987/987`, stricter Obsidian proof is `399/987`, `588` Platinum entries need Obsidian, and `0` generated N5/N4 word rows are blocked/failing structurally. The word source-posture report is scoped to the `987` structurally current-standard entries: `121/987` have independent source families proven, `866/987` are single-source-family, and `0/987` are missing governed source evidence; single-source entries carry `word_source_independence_not_proven`.
...
- `deck:words:platinum:rereview-status -- --levels=5,4` uses generated word rows as the denominator. N5 word `287/287` and N4 word `112/700` Obsidian remain separate from `987/987` structural Platinum, and `588` N5/N4 word Platinum entries still need substantive Obsidian rereview proof.
- `deck:words:platinum:certify-status -- --levels=5,4` is the fail-closed word Obsidian gate. It currently fails because `588` generated N4 word rows still need card-bound strict Obsidian proof.
- `deck:platinum:governance-gate` exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It currently passes with governance warnings for N4 word Platinum entries lacking Obsidian proof, word single-source-family posture, card-specific or bulk revalidation summaries, marker-only example-quality automation, and zero active verification limitations.
