...
npm run data:merge:jlpt:source-batch -- --source=shin_kanzen_master_kanji --batch=downloads/shin-kanzen-master-kanji-evidence-n5-source-gaps-batch-001.tsv
...
| `npm run data:audit:jlpt:source-levels -- --level=5` | Report active source-claimed N5 candidates, candidates missing from the current N5 contract, disputed missing candidates, local non-voting source-input progress, and current rows lacking same-level source support without changing decks or readiness |
...
| `npm run data:merge:jlpt:source-batch -- --source=shin_kanzen_master_kanji --batch=downloads/shin-kanzen-master-kanji-evidence-n5-source-gaps-batch-001.tsv` | Dry-run merge a reviewed local source-gap batch into its full ignored source worksheet without importing evidence or changing decks |
