...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `402` high-confidence rows, `57` standard-confidence rows, `19` disputed rows, `1734` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1648` insufficient independent source rows, `1358` insufficient independent evidence-lineage rows, `1365` missing Japanese-published source rows, `19` disputed consensus rows, and `212` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1792` rows needing governed source review: `1365` missing Japanese-published source, `201` contract/consensus mismatch, `122` insufficient independent sources, `85` weak evidence, and `19` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `314` Shin Kanzen reviewed rows plus `303` `source_access_gap` rows (`617` resolved), and `498` Sou Matome reviewed rows plus `417` `source_access_gap` rows (`915` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
