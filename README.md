...
| Surface | Current state | Main gates |
| --- | --- | --- |
| N5 word | `300/546` strict word Obsidian-certified, plus `20` tracked source-only phrase exclusions outside the generated denominator. The 246 current N5 word v2 dictionary-common-pool Silver additions are generated-ready with word audio, pitch, required back-side fields, examples, reading breakdowns, support labels, and readiness, but they have not entered Gold/Sapphire/Platinum/Obsidian. Live readiness is `ready_with_deferred_variants`; reading coverage is `69.5%` (`239/344`). Release artifact QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:obsidian:certify-status -- --levels=5`, `deck:words:ready -- --levels=5` |
...
Strict word Obsidian proof covers `1000/1265` current N5/N4 generated word rows: the N5 Obsidian-certified subset remains `300/546`, and the N4 Obsidian-certified subset remains `700/719` after the current word v2 common-word Silver additions. Native Sapphire structural coverage and current-standard Platinum card-surface inspection are complete lower-lane prerequisites for the certified subset; the 246 new N5 Silver rows and 19 new N4 Silver rows must not be described as Gold, Sapphire, Platinum, or Obsidian-certified.
...
`deck:words:platinum:source-posture -- --levels=5,4` currently inspects `1000` structurally current-standard entries: `121/1000` have independent source families proven, `879/1000` are single-source-family, and `0/1000` are missing governed source evidence.
...
- `deck:words:obsidian:rereview-status -- --levels=5,4` uses generated word rows as the denominator and defaults to canonical JSONL proof for migrated N5/N4 word ledgers. This legacy proof consumer still reports the structural column as Platinum compatibility while native `deck:words:sapphire:*` owns Sapphire coverage. The N5 word `300/546` and N4 word `700/719` Obsidian-certified subsets remain separate from current N5/N4 v2 Silver additions.
- `deck:words:obsidian:certify-status -- --levels=5,4` is the fail-closed word Obsidian gate. It is expected to fail for the current full N5/N4 generated denominator until the 246 new N5 and 19 new N4 Silver rows complete Gold, Sapphire, Platinum, and Obsidian.
