...
These commands create or extend local ignored datasets so the decks are usable before you build out full coverage. The tracked starter packs now carry complete N5 and N4 kanji curation, the first four N3 kanji starter batches, and a 109-card curated N5 starter word pack for the word deck.
...
- kanji breakdown panels on the back prefer curated kanji display words and meanings, which lets starter curation pin learner-friendly forms such as `大きい`, `小さい`, `高い`, `前`, `座る`, `閉める`, and `所` instead of raw dictionary winners
