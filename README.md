...
| N5 word | Golden-reviewed at `326/326`; `ready_with_deferred_variants` in curated-only mode; audio, pitch accent, card-back fields, example reading alignment, and looping animations are complete for shipped rows; word platinum has 208 active approvals, 22 deferred/removed rows, 118 generated rows still missing platinum approval, and no `needs_review` blockers under the generated-pitch-label policy |
...
- Current N5+N4 word rows: `784`
- N5 word reading coverage: `76.2% (262/344)` when built alone
- N5 word reading coverage: `77.6% (267/344)` when built with N4 as the selected word-product scope
