...
| `shin_kanzen_master_kanji` | [Shin Kanzen Master textbooks](https://www.3anet.co.jp/np/en/list.html?series_id=4) | Active restricted manual-citation assignment lane with `64` reviewed N4 rows from the pinned ignored worksheet; `10` checked rows are marked non-importing `source_access_gap`; additional rows remain pending until exact source-level evidence is reviewed |
