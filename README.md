...
| N5 word | Golden-reviewed at `326/326`; `ready_with_deferred_variants` in curated-only mode; audio, pitch accent, card-back fields, example reading alignment, and looping animations are complete for shipped rows; word platinum has 220 active approvals, 22 deferred/removed rows, 106 generated rows still missing platinum approval, and no `needs_review` blockers under the generated-pitch-label policy |
