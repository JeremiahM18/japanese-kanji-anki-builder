...
| N4 word | Expanded to `667` governed rows; current word-level placement audit and golden review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Current-standard word platinum has started at `96/667` under `word-platinum-v2-limitation-aware`; full N4 word platinum is not complete. |
