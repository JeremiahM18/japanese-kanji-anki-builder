...
- `npm run deck:words:ready -- --levels=5` now writes an explicit completion block into `out/word-build/build-summary.json` and the console report so we can see tracked N5 governance coverage, explicit reading-coverage contract coverage, canonical inventory counts, the live reading-audit summary (`189/344`, `54.9%` covered on the current N5 baseline), and true looping animation coverage before claiming the word deck is complete.
