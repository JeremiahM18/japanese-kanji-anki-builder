...
Reviewer evidence is stored in the source-centric `assignments[sourceId][kanji]` rows. The materialized `kanji` rollup is derived summary state and intentionally keeps only level/review-status facts plus computed consensus, agreement, confidence, and notes so reviewer citations are not duplicated across the manifest.
...
| `ask_hajimete_jlpt_kanji` | [ASK Hajimete JLPT kanji books](https://ask-books.com/series/jlpt-kanji/) | Active restricted manual-citation kanji textbook lane with `33` reviewed assignments from pinned official N1/N3 sample target-entry pages; continue only where exact N1-N3 assignment proof is available |
...
| `shin_kanzen_master_kanji` | [Shin Kanzen Master textbooks](https://www.3anet.co.jp/np/en/list.html?series_id=4) | Active restricted manual-citation assignment lane with `288` reviewed N4 rows from the pinned ignored worksheet; `316` checked rows are marked non-importing `source_access_gap`; additional rows remain pending until exact source-level evidence is reviewed |
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `564` reviewed assignments from the pinned ignored worksheet; `1648` worksheet rows remain pending |
| `try_jlpt_textbook` | [TRY! JLPT textbooks](https://ask-books.com/jlpt-try) | Blocked from assignment consensus under current source access; a 2026-05-08 source-access spike found official public TRY materials expose grammar/vocabulary surfaces, not exact per-kanji assignment proof |
...
- Current active assignment lanes are still evidence, not deck movement. `kanjidic2_legacy` has `1479` reviewed exact assignments and `0` current range rows; future old JLPT 2 rows stay N2/N3 range evidence when present. `tanos_legacy_direct` has `1478` reviewed N1/N4/N5 assignments. `tanos_estimated_split` has `734` lower-weight reviewed estimated N2/N3 assignments (`367` N2 and `367` N3) and must not settle taxonomy movement by itself. `shin_kanzen_master_kanji` has `288` active restricted manual-citation N4 assignments from the pinned ignored worksheet; `1608` worksheet rows are still pending and `316` checked rows are marked non-importing `source_access_gap` until fuller source access can provide exact source-level evidence. `nihongo_sou_matome_kanji` has `564` active restricted manual-citation assignments from the pinned ignored worksheet; `1648` worksheet rows are still pending. `ask_hajimete_jlpt_kanji` has `33` active restricted manual-citation N1/N3 assignments from pinned official sample target-entry pages. `official_jlpt_sample_workbooks` is active occurrence-only evidence backed by [templates/jlpt_official_kanji_occurrences.json](templates/jlpt_official_kanji_occurrences.json), and official occurrence rows may store only level, source PDF, section, page, question reference, and observed kanji.
- Restricted or non-assignment lanes remain fenced. `tanos_frequency_method_notes` is active non-voting methodology evidence. `ask_hajimete_jlpt_kanji` is active only for pinned, reviewed N1-N3 manual-citation rows with exact assignment proof. `jlptsensei` is a registered restricted manual-citation lane but remains inactive until reviewed rows, pinned source-input integrity, and explicit activation are in place. `try_jlpt_textbook` is blocked from assignment consensus under current source access because official public TRY materials expose grammar/vocabulary surfaces, not exact per-kanji assignment proof. `japanese_textbook_consensus` is an active derived non-voting summary computed from the individual textbook lanes. `kanshudo` and `wanikani` are blocked from consensus until a governed use path exists; `jpdb` is frequency-sanity only. Planned legal/free sanity lanes include BCCWJ frequency sanity checks and Kanji Alive background metadata.
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `298` high-confidence rows, `62` standard-confidence rows, `19` disputed rows, `1833` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1760` insufficient independent source rows, `1487` insufficient independent evidence-lineage rows, `1495` missing Japanese-published source rows, `19` disputed consensus rows, and `208` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1881` rows needing governed source review: `1495` missing Japanese-published source, `197` contract/consensus mismatch, `100` insufficient independent sources, `70` weak evidence, and `19` disputed consensus, with source-input progress at `33` ASK Hajimete reviewed rows (`33` resolved), `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved), and `564` Sou Matome reviewed rows (`564` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
...
`data:benchmark:jlpt:sources` is a read-only cost report for the JLPT kanji source-evidence workflow. It measures source-input preflight, source-input import dry-run/materialization, full evidence-manifest serialization, and source-evidence audit timing while also reporting file sizes, line counts, assignment counts, rollup count, and selected-source citation repetition. Use it before choosing performance refactors such as source-assignment splitting, citation defaulting, or audit caching. It does not import assignments, move kanji, move words, update decks, or change readiness.
...
| `npm run data:merge:jlpt:source-batch -- --source=<source-id> --batch=<ignored-batch.tsv>` | Dry-run merge a reusable local source-decision batch into its full ignored source worksheet without importing evidence or changing decks |
...
| `npm run data:template:jlpt:textbook-source -- --source=<source-id> --priority=source-review-worklist --limit=10 --out=<ignored-batch.tsv>` | Create or overwrite one reusable ignored all-level manual-review batch for a Japanese-published textbook source lane, ordered by source-lane batch value |
