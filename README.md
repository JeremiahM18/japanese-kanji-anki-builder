...
Word Platinum uses `word-platinum-v3-evidence-lanes`. Current N5/N4 generated word denominator is `987` rows. Structural Platinum is `954/987`, Obsidian is `8/987`, `946` Platinum entries need Obsidian, and `33` generated N4 rows are blocked/failing because they are newly routed Silver rows without Gold or current-standard Platinum entries yet. The word source-posture report is scoped to the `954` structurally current-standard entries only: `115/954` have independent source families proven, `839/954` are single-source-family, and `0/954` are missing governed source evidence; single-source entries carry `word_source_independence_not_proven`.
