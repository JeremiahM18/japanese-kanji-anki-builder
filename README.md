...
| N4 word | Expanded to `667` governed rows; current word-level placement audit and Gold review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Platinum word review exists for `96/667`, but `0/667` generated N4 word rows are Obsidian certified; the `96` Platinum entries need Obsidian review and the other `571` generated rows still lack active current-standard structural entries. Full N4 word Obsidian certification is not complete. |
...
- N5 word golden review passes against the current `287` generated rows, and the default current-standard word Platinum command passes at `287/287`. N4 word golden review passes at `667/667` generated rows; N4 current-standard word Platinum coverage is partial at `96/667`, with `571` generated rows still missing current-standard review. Gold/Platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, listening review, and Obsidian certification.
- Current kanji Platinum standard is `kanji-platinum-v3-evidence-lanes`. Only current-standard `platinum` and `fixed_then_platinum` kanji manifest entries count as active Platinum coverage until explicit non-mechanical rereview provenance certifies them as Obsidian. Legacy/unversioned kanji review history must be marked `needs_revalidation`; it is non-certifying backlog/history and still counts as missing Platinum coverage until revalidated. Kanji `sourceEvidence` is reserved for governed Japanese-source field truth; golden review, generated surface, media, audio, stroke-order, and manual review checks live in separate evidence lanes.
- `deck:platinum:rereview-status -- --levels=5,4` is the conservative Obsidian provenance report. Current N5/N4 output is `292/292` Platinum pass, `170/292` Obsidian certified, `122/292` Platinum entries needing Obsidian, and `0/292` blocked/failing. Its searchable limitation marker is `missing_substantive_current_standard_rereview_proof`.
- `deck:kanji:platinum:certify-status -- --levels=5,4` is the fail-closed kanji Obsidian certification gate. It currently fails because N4 has `122` Platinum entries needing Obsidian; failures list the card, field, expected value, actual state, evidence lane, and reviewer action. The certification scope is recorded as `best_effort_non_native_review` unless later fluent/native audit provenance exists.
...
- `deck:words:platinum:rereview-status -- --levels=5,4` is the conservative Obsidian provenance report. Current N5/N4 output is `954` generated deck rows, `383/954` Platinum pass, `8/954` Obsidian certified, `375` Platinum entries needing Obsidian, and `571/954` blocked/failing because N4 word Platinum coverage is incomplete. For N4 word specifically, the Obsidian count is `0/667`; the `96` Platinum entries do not shrink the square-zero Obsidian denominator. Its searchable limitation marker is `missing_substantive_current_standard_word_rereview_proof`.
