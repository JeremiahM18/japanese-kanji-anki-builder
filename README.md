...
Word Platinum uses `word-platinum-v3-evidence-lanes`. Current N5/N4 generated word denominator is `987` rows. Structural Platinum is `954/987`, Obsidian is `287/987`, `667` Platinum entries need Obsidian, and `33` generated N4 rows are blocked/failing because they are Gold-reviewed rows without current-standard Platinum entries yet. The word source-posture report is scoped to the `954` structurally current-standard entries only: `121/954` have independent source families proven, `833/954` are single-source-family, and `0/954` are missing governed source evidence; single-source entries carry `word_source_independence_not_proven`.
...
- `deck:words:platinum:rereview-status -- --levels=5,4` uses generated word rows as the denominator. N4 word `0/700` Obsidian remains separate from `667/700` structural Platinum; the 33 newly routed N4 Gold-reviewed rows are blocked/failing until Platinum review entries are added.
- `deck:platinum:governance-gate` exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It currently passes with an `allowed_incomplete_word_platinum_level` warning for the 33 newly routed N4 Gold-reviewed rows that do not yet have Platinum review coverage, plus the existing governance warnings for missing Obsidian proof, word single-source-family posture, card-specific or bulk revalidation summaries, marker-only example-quality automation, and zero active verification limitations.
