...
| Surface | Status |
| --- | --- |
| N5 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `80/80`, but only `60/80` have explicit non-mechanical substantive rereview proof and `20/80` remain in the square-zero rereview queue |
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `212/212`, but `0/212` have explicit non-mechanical substantive rereview proof, so all `212` N4 kanji remain in the square-zero rereview queue |
...
| N4 word | Expanded to `667` governed rows; current word-level placement audit and golden review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Current-standard structural word review exists for `96/667`, but `0/667` generated N4 word rows have explicit non-mechanical substantive rereview proof; the `96` structural-only entries need rereview and the other `571` generated rows still lack active current-standard structural entries. Full N4 word platinum is not complete. |
...
- N5 word golden review passes against the current `287` generated rows, and the default current-standard word platinum command passes at `287/287`. N4 word golden review passes at `667/667` generated rows; N4 current-standard word platinum is partial at `96/667`, with `571` generated rows still missing current-standard platinum review. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
- Current kanji platinum standard is `kanji-platinum-v3-evidence-lanes`. Only current-standard `platinum` and `fixed_then_platinum` kanji entries count as active platinum coverage. Legacy/unversioned kanji review history must be marked `needs_revalidation`; it is non-certifying backlog/history and still counts as missing platinum coverage until revalidated. Kanji `sourceEvidence` is reserved for governed Japanese-source field truth; golden review, generated surface, media, audio, stroke-order, and manual review checks live in separate evidence lanes.
- `deck:platinum:rereview-status -- --levels=5,4` is the conservative kanji rereview provenance report. Current N5/N4 output is `292/292` structural v3 gate pass, `60/292` explicit substantive current-standard rereview proven, `232/292` structural-only entries needing substantive rereview, and `0/292` blocked/failing. Its searchable limitation marker is `missing_substantive_current_standard_rereview_proof`.
- Current word platinum standard is `word-platinum-v3-evidence-lanes`. Active word platinum entries without `reviewStandard`, `revalidatedAt`, `revalidationSummary`, `notesIncludes`, governed `sourceEvidence`, `internalChecks`, and `reviewEvidence` are reported as legacy/unversioned and do not pass the default platinum commands. Word `sourceEvidence` is reserved for external Japanese-source truth evidence; golden review is enforced as an internal regression check, not source proof. Use `--allow-legacy-standard` only for historical inspection while revalidating.
- `deck:words:platinum:rereview-status -- --levels=5,4` is the conservative word rereview provenance report. Current N5/N4 output is `954` generated deck rows, `383/954` structural v3 gate pass, `8/954` explicit substantive current-standard rereview proven, `375` structural-only entries needing substantive rereview, and `571/954` blocked/failing because N4 word structural platinum coverage is incomplete. For N4 word specifically, the substantive proof count is `0/667`; the `96` old structural entries do not shrink the square-zero rereview denominator. Its searchable limitation marker is `missing_substantive_current_standard_word_rereview_proof`.
...
`deck:kanji:review-status` reports generated, golden, active current-standard structural platinum coverage, non-certifying revalidation backlog/history, and structured verification-limitation counts for the five core kanji decks and five `additional_unverified_Nx` decks. It also fails on unresolved duplicate additional source claims so duplicate notes cannot silently enter optional decks.
...
Golden review protects learner-facing card output rather than raw field presence. Platinum review is the final release-quality pass. `deck:platinum:batch` is a read-only pre-review report for kanji cards: it shows the generated surface, hard-rule checks, risk flags, current structural/review status, and next substantive rereview queue before any manifest edits. `deck:words:platinum:batch` provides the matching read-only packet for word cards: it shows exact written-reading identity, generated card fields, sentence lines, exact word audio, pitch source/render status, lookup links, risk flags, and a suggested review step. Batch reports do not create platinum entries or prove release readiness. Use `--queue=missing-current-standard` only when intentionally inspecting structural coverage gaps. The `npm run deck:platinum:n5` and `npm run deck:words:platinum:n5` commands are full-level structural gates and fail until every generated N5 card has an active current-standard structural entry.
...
| `npm run deck:kanji:review-status` | Report core/additional kanji generated, golden, current-standard platinum, revalidation backlog, and duplicate-claim status |
...
| `npm run deck:words:platinum:n5` | Run the N5 word current-standard platinum release-quality benchmark |
| `npm run deck:words:platinum:n4` | Run the N4 word current-standard platinum release-quality benchmark |
| `npm run deck:words:platinum:rereview-status -- --levels=5,4` | Classify word platinum structural pass versus explicit substantive rereview proof |
