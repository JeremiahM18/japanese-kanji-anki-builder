...
| N4 word | Expanded to `667` governed rows; current word-level placement audit and golden review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants; N4 word platinum has not started. |
...
- N5 word golden review passes against the current `287` generated rows, and the default current-standard word platinum command passes at `287/287`. N4 word golden review passes at `667/667` generated rows after refreshing stale breakdown expectations and fixing the `行う|おこなう` word breakdown; N4 platinum has not started. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
