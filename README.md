...
| N5 word | Golden-reviewed at `331/331`; `ready_with_deferred_variants` in curated-only mode; audio, pitch accent, card-back fields, example reading alignment, and looping animations are complete for shipped rows; word platinum has 321 active approvals, 53 deferred/removed rows, 10 generated rows still missing platinum approval, and no `needs_review` blockers under the generated-pitch-label policy |
...
- N5 word reading coverage: `68% (234/344)` when built alone
- N5 word reading coverage: `69.5% (239/344)` when built with N4 as the selected word-product scope
