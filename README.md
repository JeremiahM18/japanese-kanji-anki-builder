...
| Surface | Status |
| --- | --- |
| N5 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; legacy/unversioned platinum entries are `80/80`, but current-standard revalidation is `0/80` under `kanji-platinum-v2-limitation-aware` |
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; legacy/unversioned platinum review is in progress with `42/212` active entries and `170` remaining, but current-standard revalidation is `0/212` |
...
- N5 word golden and platinum checks pass against the current `287` generated rows. N4 word golden review passes against the current `667` generated rows, but N4 platinum has not started. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
- Current kanji platinum standard is `kanji-platinum-v2-limitation-aware`. Active kanji platinum entries without `reviewStandard`, `revalidatedAt`, and `revalidationSummary` are reported as legacy/unversioned and do not pass the default platinum commands. Use `--allow-legacy-standard` only for historical inspection while revalidating.
...
- Platinum manifests are in progress. Only active `platinum` and `fixed_then_platinum` entries count as reviewed release coverage; for kanji release coverage they must also pass the current `kanji-platinum-v2-limitation-aware` standard.
- Kanji platinum entries created before the current versioned standard are legacy/unversioned history, not trusted release coverage, until they are revalidated and pass the default platinum command.
