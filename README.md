...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `544` reviewed assignments from the pinned ignored worksheet; `1668` worksheet rows remain pending |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `263` high-confidence rows, `62` standard-confidence rows, `19` disputed rows, `1868` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1795` insufficient independent source rows, `1524` insufficient independent evidence-lineage rows, `1532` missing Japanese-published source rows, `19` disputed consensus rows, and `206` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1916` rows needing governed source review: `1532` missing Japanese-published source, `195` contract/consensus mismatch, `100` insufficient independent sources, `70` weak evidence, and `19` disputed consensus, with source-input progress at `15` ASK Hajimete reviewed rows (`15` resolved), `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved), and `544` Sou Matome reviewed rows (`544` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
