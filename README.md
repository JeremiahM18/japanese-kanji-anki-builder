...
| N4 word | `667` canonical rows. Gold, word-level placement, readiness, and Platinum pass at `667/667`; expansion signals are exhausted with reading coverage `76.6%` (`578/755`) and only deferred variants remaining. Obsidian is `0/667`, so all N4 word rows remain in the square-zero Obsidian queue. | `deck:words:platinum:n4`, `deck:words:completion:n4`, `deck:words:gap-plan:n4`, `deck:words:expansion-signals -- --levels=5,4` |
...
Current N5/N4 source-list routing status: N5 enhancement candidates are exhausted for current-level promotion and N5 cross-level routing is fully triaged at `151/151` rows (`141` move candidates, `10` rejected rows, `0` untriaged). N4 enhancement candidates are also exhausted for current-level promotion, while known-JLPT rows with no N4 anchor are tracked separately at `252/256` cross-level routing rows triaged (`238` move candidates, `11` rejected rows, `3` deferred rows, `4` advisory rows still untriaged). These routing rows do not reopen N5/N4 current-level expansion and do not move cards by themselves.
