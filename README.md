...
| Surface | Status |
| --- | --- |
| N5 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural platinum gates pass at `80/80`, but `deck:platinum:rereview-status -- --levels=5` classifies `0/80` with explicit non-mechanical substantive rereview proof |
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural platinum gates pass at `212/212`, but `deck:platinum:rereview-status -- --levels=4` classifies `0/212` with explicit non-mechanical substantive rereview proof |
...
| N5 word | Expanded to `287` canonical governed rows plus `20` tracked source-only phrase exclusions; current word-level placement, golden, tracked-source artifact, automated readiness, and current-standard word platinum checks pass at `287/287` under `word-platinum-v3-evidence-lanes`, but `deck:words:platinum:rereview-status -- --levels=5` classifies `0/287` with explicit non-mechanical substantive rereview proof. N5 word release still needs manual import QA, accessibility, and listening checks before release-ready status. |
| N4 word | Expanded to `667` governed rows; current word-level placement audit and golden review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Current-standard word platinum has started at `96/667` under `word-platinum-v3-evidence-lanes`; `deck:words:platinum:rereview-status -- --levels=4` classifies `0/96` structurally passing entries with explicit non-mechanical substantive rereview proof and `571` generated rows as blocked/failing due missing active current-standard platinum entries. Full N4 word platinum is not complete. |
...
- `deck:platinum:rereview-status -- --levels=5,4` is the conservative kanji rereview provenance report. Current N5/N4 output is `292/292` current v3 structural pass, `0/292` explicit substantive current-standard rereview proven, `292/292` needs substantive rereview, and `0/292` blocked/failing. Its searchable limitation marker is `missing_substantive_current_standard_rereview_proof`.
...
- `deck:words:platinum:rereview-status -- --levels=5,4` is the conservative word rereview provenance report. Current N5/N4 output is `383/954` current v3 structural pass, `0/383` explicit substantive current-standard rereview proven, `383/383` structurally passing entries need substantive rereview, and `571/954` blocked/failing because N4 word platinum is incomplete. Its searchable limitation marker is `missing_substantive_current_standard_word_rereview_proof`.
- `deck:words:platinum:source-posture -- --levels=5,4` is the word source-family posture report. Current N5/N4 active current-standard word platinum has `115/383` entries with independent source families proven, `268/383` single-source-family entries, and `0/383` missing governed sources. Single-source-family entries are structurally governed but carry the searchable marker `word_source_independence_not_proven`; word source-claim origin independence remains explicitly `word_source_claim_origin_independence_not_evaluated` until a word source-origin manifest exists.
- `deck:platinum:governance-gate` is a local-data gate that exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It fails dirty reviewed entries and missing governed word sources, but allows the known incomplete N4 word surface only when every blocked row is missing active current-standard platinum coverage. Current output is passing with governance warnings for missing substantive rereview proof, bulk-template revalidation summaries, marker-only example-quality automation, zero active verification limitations, N4 word incompleteness, and word single-source-family posture.
