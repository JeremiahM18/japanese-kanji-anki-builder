...
- For broader beginner kanji, the starter curation now mixes bare-kanji and learner-word anchors intentionally: cards like `力 (ちから)`, `場 (ば)`, `用 (よう)`, and `間 (あいだ)` now expose the kanji-level hook directly, while `食` explicitly prefers `食べる` because that is the clearest first-use anchor for beginners.
