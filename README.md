...
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current-standard platinum is `60/212`; `42` legacy/unversioned review-history entries are marked `needs_revalidation` and non-certifying; `152` cards still need current-standard platinum coverage |
...
| N4 word | Expanded to `667` governed rows; current word-level placement, golden review, and automated card-field/audio/pitch readiness checks pass against the generated N4 surface. N4 word completion is `ready_with_deferred_variants`: the active reading backlog is cleared and the remaining open reading items are explicitly deferred variants. Platinum review, import QA, accessibility, and listening checks are still required. |
...
- Word reading coverage from current completion reports is N5 `233/344` (`67.7%`) and N4 `579/765` (`75.7%`). N5 and N4 are `ready_with_deferred_variants`; N4 active reading backlog is `0`, deferred variants are `186`, and configured source-enhancement keep and untriaged candidates are `0/0`.
- N5 word golden review passes against the current `287` generated rows. N5 legacy/unversioned word platinum passes only with `--allow-legacy-standard`; the default current-standard word platinum command fails at `8/287` current-standard entries and `279` legacy/unversioned active entries remaining. N4 word golden review passes against the current `667` generated rows, but N4 platinum has not started. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
