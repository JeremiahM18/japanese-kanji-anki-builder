...
`deck:kanji:partition-plan` converts the source-level delta audit into a read-only kanji product plan: five core logical decks plus five `additional_unverified_Nx` logical decks. By default it includes only learner-useful earlier-placement candidates, excludes disputed rows, and reports duplicate-note risk before any physical export work. It does not move contracts, generate decks, import evidence, or change readiness.
