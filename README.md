...
| N4 word | `675` canonical rows after the first routed N5-source move batch. The Silver/generated card surface builds at `675/675` with word audio, pitch, required back-side fields, word-level placement, examples, and labels complete; completion is `ready_with_deferred_variants` with reading coverage `76.6%` (`578/755`) and only deferred variants remaining. Gold and Platinum still cover `667/675`; the 8 new routed rows are not Gold, Platinum, or Obsidian yet. Obsidian is `0/675`. | `deck:words:ready -- --levels=4`, `deck:words:completion:n4`, `deck:words:review:n4`, `deck:words:platinum:n4`, `deck:words:expansion-signals -- --levels=5,4` |
...
Word Platinum uses `word-platinum-v3-evidence-lanes`. Current N5/N4 generated word denominator is `962` rows. Structural Platinum is `954/962`, Obsidian is `8/962`, `946` Platinum entries need Obsidian, and `8` generated N4 rows are blocked/failing because they are newly routed Silver rows without Gold or current-standard Platinum entries yet. The word source-posture report is scoped to the `954` structurally current-standard entries only: `115/954` have independent source families proven, `839/954` are single-source-family, and `0/954` are missing governed source evidence; single-source entries carry `word_source_independence_not_proven`.

Current N5/N4 source-list routing status: N5 enhancement candidates are exhausted for current-level promotion and N5 cross-level routing is fully triaged at `151/151` rows (`141` move candidates, `10` rejected rows, `0` untriaged). Eight N5-source move candidates have now been physically routed into the N4 word contract/starter data; the remaining routed target-level backlog is N4 `25`, N3 `212`, N2 `97`, and N1 `41`. N4 enhancement candidates are also exhausted for current-level promotion, while known-JLPT rows with no N4 anchor are tracked separately and fully triaged at `256/256` cross-level routing rows (`242` move candidates, `11` rejected rows, `3` deferred rows, `0` untriaged). These routing rows do not reopen N5/N4 current-level expansion and do not move cards by themselves.
...
- `deck:words:platinum:rereview-status -- --levels=5,4` uses generated word rows as the denominator. N4 word `0/675` Obsidian remains separate from `667/675` structural Platinum; the 8 newly routed N4 Silver rows are blocked/failing until Gold and Platinum review entries are added.
- `deck:platinum:governance-gate` exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It currently passes with an `allowed_incomplete_word_platinum_level` warning for the 8 newly routed N4 Silver rows that do not yet have Gold/Platinum review coverage, plus the existing governance warnings for missing Obsidian proof, word single-source-family posture, card-specific or bulk revalidation summaries, marker-only example-quality automation, and zero active verification limitations.
