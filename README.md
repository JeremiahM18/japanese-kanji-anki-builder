...
| Surface | Current state | Main gates |
| --- | --- | --- |
| N5 word | `10/588` current word Obsidian v2.5-certified. N5 Gold, Sapphire, and current-standard Platinum are complete at `588/588`; the remaining `578` N5 rows need Obsidian v2.5 rereview, including real example-sentence quality review before example-sentence audio and governed JSONL proof. Legacy N5 proof history remains audit-visible for `418` targets, with `10` superseded by current v2.5 proof; it is not current certification. No N5 source-only phrase exclusions remain in the tracked word contract. Live readiness is `ready_with_deferred_variants`; reading coverage is `71.2%` (`245/344`). Release artifact QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:obsidian:certify-status -- --levels=5`, `deck:words:ready -- --levels=5` |
...
Current word Obsidian v2.5 proof covers `10/1307` current N5/N4 generated word rows: N5 is `10/588`, and N4 is `0/719`. Legacy word Obsidian proof history remains audit-visible at `1118` N5/N4 targets and `1128` raw ledger events; `10` legacy events are superseded by current v2.5 proof. Legacy history does not count as current certification. Native Sapphire structural coverage and current-standard Platinum card-surface inspection are lower-lane prerequisites only; the remaining rows must not be described as current Obsidian v2.5-certified.
...
- `deck:words:obsidian:rereview-status -- --levels=5,4` uses generated word rows as the denominator and defaults to canonical JSONL proof for migrated N5/N4 word ledgers. It reports current word Obsidian v2.5 proof only when the card has `word-obsidian-v2.5-sentence-audio`, exact identity binding, real sentence-quality review, and exact example-sentence audio proof. Current N5/N4 v2.5 certification is `10/1307`; the older `1118` target proof history is legacy audit history.
- `deck:words:obsidian:certify-status -- --levels=5,4` is the fail-closed word Obsidian gate. It is expected to fail for the current full N5/N4 generated denominator until the remaining `578` N5 Platinum-passing rows and `700` N4 Platinum-passing rows complete Obsidian v2.5 proof, and the 19 N4 Silver rows complete Gold, Sapphire, Platinum, and Obsidian.
