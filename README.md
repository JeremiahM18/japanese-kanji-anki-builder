...
| Surface | Status |
| --- | --- |
| N5 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `80/80`, and `80/80` now have explicit non-mechanical substantive rereview proof |
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `212/212`, and `90/212` now have explicit non-mechanical substantive rereview proof; the remaining `122` N4 kanji stay in the square-zero rereview queue |
...
| Additional kanji source-claim diagnostic | There are currently `0` physical additional kanji cards. The old "additional" surface is now a governed source-claim diagnostic: `deck:kanji:additional:ready` passes with `421` raw source claims, `0` selected additional cards, `406` unique core-retained kanji, and `421` suppressed source claims. Current `deck:kanji:review-status` passes with all additional generated/golden/platinum counts at `0/0`; `15` duplicate additional-source kanji remain tracked in the report with `0` unresolved duplicates. Future additional cards may exist only if a genuinely non-core candidate is selected and reviewed as a separate optional `additional_unverified_Nx` deck. |
| N5 word | Expanded to `287` canonical governed rows plus `20` tracked source-only phrase exclusions; current word-level placement, golden, tracked-source artifact, automated readiness, and current-standard structural word checks pass at `287/287`, but only `8/287` have explicit non-mechanical substantive rereview proof and `279/287` remain in the square-zero rereview queue. N5 word release still needs manual import QA, accessibility, and listening checks before release-ready status. |
| N4 word | Expanded to `667` governed rows; current word-level placement audit and Gold review pass, but current completion/readiness is still `incomplete`. `deck:words:gap-plan:n4` shows one active planning item (`元 -> げん`) and `178` hidden deferred variants. Platinum Candidate word review exists for `96/667`, but `0/667` generated N4 word rows are Platinum certified; the `96` candidate-only entries need certification and the other `571` generated rows still lack active current-standard structural entries. Full N4 word Platinum certification is not complete. |

Current tracked word inventory:

- N5 canonical word rows: `287`
- N5 source-only phrase exclusions: `20`
- N4 canonical word rows: `667`
- N3 canonical word rows: `19`
- N2 canonical word rows: `18`
- N1 canonical word rows: `16`
- Current N5+N4 word rows: `954`
- Word-level placement audit passes: `0/1007` canonical rows. By level: N5 `0/287`, N4 `0/667`, N3 `0/19`, N2 `0/18`, N1 `0/16`.
- Word reading coverage from current completion reports is N5 `233/344` (`67.7%`) and N4 `577/756` (`76.3%`). N5 is `ready_with_deferred_variants`; N4 is currently `incomplete` with `1` active planning item (`元 -> げん`) and `178` hidden deferred variants in `deck:words:gap-plan:n4`.
- N5 word golden review passes against the current `287` generated rows, and the default current-standard word Platinum Candidate command passes at `287/287`. N4 word golden review passes at `667/667` generated rows; N4 current-standard word Platinum Candidate coverage is partial at `96/667`, with `571` generated rows still missing current-standard candidate review. Gold/Platinum Candidate commands are still separate from APKG import QA, manual card QA, accessibility checks, listening review, and Platinum certification.
- Current kanji Platinum Candidate standard is `kanji-platinum-v3-evidence-lanes`. Only current-standard `platinum` and `fixed_then_platinum` kanji manifest entries count as active candidate coverage until explicit non-mechanical rereview provenance certifies them as Platinum. Legacy/unversioned kanji review history must be marked `needs_revalidation`; it is non-certifying backlog/history and still counts as missing candidate coverage until revalidated. Kanji `sourceEvidence` is reserved for governed Japanese-source field truth; golden review, generated surface, media, audio, stroke-order, and manual review checks live in separate evidence lanes.
- `deck:platinum:rereview-status -- --levels=5,4` is the conservative kanji certification provenance report. Current N5/N4 output is `292/292` Platinum Candidate pass, `170/292` Platinum certified, `122/292` candidate-only entries needing certification, and `0/292` blocked/failing. Its searchable limitation marker is `missing_substantive_current_standard_rereview_proof`.
- Current word Platinum Candidate standard is `word-platinum-v3-evidence-lanes`. Active word platinum entries without `reviewStandard`, `revalidatedAt`, `revalidationSummary`, `notesIncludes`, governed `sourceEvidence`, `internalChecks`, and `reviewEvidence` are reported as legacy/unversioned and do not pass the default candidate commands. Word `sourceEvidence` is reserved for external Japanese-source truth evidence; golden review is enforced as an internal regression check, not source proof. Use `--allow-legacy-standard` only for historical inspection while revalidating.
- `deck:words:platinum:rereview-status -- --levels=5,4` is the conservative word certification provenance report. Current N5/N4 output is `954` generated deck rows, `383/954` Platinum Candidate pass, `8/954` Platinum certified, `375` candidate-only entries needing certification, and `571/954` blocked/failing because N4 word candidate coverage is incomplete. For N4 word specifically, the certified count is `0/667`; the `96` old structural entries do not shrink the square-zero certification denominator. Its searchable limitation marker is `missing_substantive_current_standard_word_rereview_proof`.
- `deck:words:platinum:source-posture -- --levels=5,4` is the word source-family posture report for the `383` structurally current-standard word entries only. It reports `115/383` entries with independent source families proven, `268/383` single-source-family entries, and `0/383` missing governed sources. These counts are source-governance diagnostics, not a rereview selection pool and not substantive platinum proof. Single-source-family entries are structurally governed but carry the searchable marker `word_source_independence_not_proven`; word source-claim origin independence remains explicitly `word_source_claim_origin_independence_not_evaluated` until a word source-origin manifest exists.
- `deck:platinum:governance-gate` is a local-data gate that exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It fails dirty reviewed entries and missing governed word sources, but allows the known incomplete N4 word surface only when every blocked row is missing active current-standard structural coverage. Current output is passing with governance warnings for missing substantive rereview proof, bulk-template revalidation summaries, marker-only example-quality automation, zero active verification limitations, N4 word incompleteness, and word single-source-family posture.
...
Silver means a generated learner-facing surface exists. Gold means golden regression protects that surface. Platinum Candidate means the current-standard structural gate passes. Platinum means explicit non-mechanical current-version rereview provenance exists. Obsidian is reserved for a future fluent/native-level language audit and is not claimed by the current best-effort review program. `deck:platinum:batch` is a read-only pre-review report for kanji cards: it shows the generated surface, hard-rule checks, risk flags, current candidate/certification status, and next certification queue before any manifest edits. `deck:words:platinum:batch` provides the matching read-only packet for word cards: it shows exact written-reading identity, generated card fields, sentence lines, exact word audio, pitch source/render status, lookup links, risk flags, and a suggested review step. Batch reports do not create Platinum Candidate entries or prove release readiness. Use `--queue=missing-current-standard` only when intentionally inspecting structural coverage gaps. The `npm run deck:platinum:n5` and `npm run deck:words:platinum:n5` commands are full-level Platinum Candidate gates and fail until every generated N5 card has an active current-standard structural entry.
...
| `npm run deck:platinum:batch -- --level=5 --limit=12` | Build a read-only kanji Platinum certification pre-review packet |
...
| `npm run deck:kanji:review-status` | Report core/additional kanji generated, Gold, Platinum Candidate, revalidation backlog, and duplicate-claim status |
...
| `npm run deck:platinum:n5` | Run the N5 kanji Platinum Candidate gate |
| `npm run deck:platinum:n4` | Run the N4 kanji Platinum Candidate gate |
| `npm run deck:platinum:n3` | Run the N3 kanji Platinum Candidate gate |
| `npm run deck:platinum:n2` | Run the N2 kanji Platinum Candidate gate |
| `npm run deck:platinum:n1` | Run the N1 kanji Platinum Candidate gate |
| `npm run deck:words:ready` | Build and package word TSV artifacts |
| `npm run deck:words:apkg` | Build word `.apkg` artifacts |
| `npm run deck:words:platinum:batch -- --level=5 --limit=8` | Build a read-only word Platinum certification pre-review packet |
| `npm run deck:words:review:n5` | Run the N5 word golden benchmark |
| `npm run deck:words:platinum:n5` | Run the N5 word Platinum Candidate gate |
| `npm run deck:words:platinum:n4` | Run the N4 word Platinum Candidate gate |
| `npm run deck:words:platinum:rereview-status -- --levels=5,4` | Classify word Platinum Candidate pass versus Platinum certification proof |
| `npm run deck:words:platinum:source-posture -- --levels=5,4` | Classify active word Platinum Candidate source-family independence posture |
