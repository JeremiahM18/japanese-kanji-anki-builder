...
| `ask_hajimete_jlpt_kanji` | [ASK Hajimete JLPT kanji books](https://ask-books.com/series/jlpt-kanji/) | Active restricted manual-citation kanji textbook lane with `208` reviewed assignments from pinned official N1/N3 sample target-entry and index pages plus exact N2 and N5 checklist pages; N4 remains unsupported until exact assignment proof is verified |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `390` high-confidence rows, `79` standard-confidence rows, `25` disputed rows, `1718` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1619` insufficient independent source rows, `1291` insufficient independent evidence-lineage rows, `1298` missing Japanese-published source rows, `25` disputed consensus rows, and `238` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1774` rows needing governed source review: `1298` missing Japanese-published source, `229` contract/consensus mismatch, `131` insufficient independent sources, `91` weak evidence, and `25` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved), and `673` Sou Matome reviewed rows plus `185` `source_access_gap` rows (`858` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
