...
| `ask_hajimete_jlpt_kanji` | [ASK Hajimete JLPT kanji books](https://ask-books.com/series/jlpt-kanji/) | Active restricted manual-citation kanji textbook lane with `15` reviewed N3 assignments from pinned official sample target-entry pages; continue only where exact N1-N3 assignment proof is available |
...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `426` reviewed assignments from the pinned ignored worksheet; `1786` worksheet rows remain pending |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `270` high-confidence rows, `52` standard-confidence rows, `12` disputed rows, `1878` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1809` insufficient independent source rows, `1577` insufficient independent evidence-lineage rows, `1585` missing Japanese-published source rows, `12` disputed consensus rows, and `197` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1920` rows needing governed source review: `1585` missing Japanese-published source, `184` contract/consensus mismatch, `72` insufficient independent sources, `67` weak evidence, and `12` disputed consensus, with source-input progress at `15` ASK Hajimete reviewed rows (`15` resolved), `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved), and `426` Sou Matome reviewed rows (`426` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
