...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `475` reviewed assignments and `431` non-importing `source_access_gap` rows; `1306` worksheet rows remain pending; continue targeted review only where exact assignment proof is available |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `399` high-confidence rows, `53` standard-confidence rows, `19` disputed rows, `1741` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1655` insufficient independent source rows, `1373` insufficient independent evidence-lineage rows, `1380` missing Japanese-published source rows, `19` disputed consensus rows, and `206` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1799` rows needing governed source review: `1380` missing Japanese-published source, `195` contract/consensus mismatch, `120` insufficient independent sources, `85` weak evidence, and `19` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `314` Shin Kanzen reviewed rows plus `303` `source_access_gap` rows (`617` resolved), and `475` Sou Matome reviewed rows plus `431` `source_access_gap` rows (`906` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
