...
| `shin_kanzen_master_kanji` | [Shin Kanzen Master textbooks](https://www.3anet.co.jp/np/en/list.html?series_id=4) | Active restricted manual-citation assignment lane with `314` reviewed assignments and `303` non-importing `source_access_gap` rows; `1595` worksheet rows remain pending until exact source-level evidence is reviewed |
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `442` reviewed assignments and `451` non-importing `source_access_gap` rows; `1319` worksheet rows remain pending; broad review is paused until fuller exact assignment access or targeted citations are available |
...
| `joyo_grade` | [Agency for Cultural Affairs Joyo kanji index](https://www.bunka.go.jp/seisaku/kokugo_nihongo/kokugo_shisaku/joyokanjihyo_sakuin/index.html) | Registered background metadata only; not JLPT assignment proof |
...
| `jpdb` | [jpdb kanji metadata](https://jpdb.io/) | Possible frequency sanity lane pending governed source-use review; not assignment truth |
| `kanshudo` | [Kanshudo terms](https://www.kanshudo.com/tc.html) | Blocked until source-use terms and a governed use path are approved |
| `wanikani` | [WaniKani terms](https://www.wanikani.com/terms) | Blocked until source-use/API/export terms and a governed use path are approved |
...
- Current active assignment lanes are still evidence, not deck movement. `kanjidic2_legacy` has `1479` reviewed exact assignments and `0` current range rows; future old JLPT 2 rows stay N2/N3 range evidence when present. `tanos_legacy_direct` has `1478` reviewed N1/N4/N5 assignments. `tanos_estimated_split` has `734` lower-weight reviewed estimated N2/N3 assignments (`367` N2 and `367` N3) and must not settle taxonomy movement by itself. `shin_kanzen_master_kanji` has `314` active restricted manual-citation assignments; `1595` worksheet rows are still pending and `303` checked rows are marked non-importing `source_access_gap` until fuller source access can provide exact source-level evidence. `nihongo_sou_matome_kanji` has `442` active restricted manual-citation assignments; `1319` worksheet rows are still pending and `451` checked rows are marked non-importing `source_access_gap`; broad Sou review is paused until fuller exact assignment access or targeted citations are available. `ask_hajimete_jlpt_kanji` has `208` active restricted manual-citation assignments from pinned official N1/N3 sample target-entry and index pages plus exact N2 and N5 checklist pages. `official_jlpt_sample_workbooks` is active occurrence-only evidence backed by [templates/jlpt_official_kanji_occurrences.json](templates/jlpt_official_kanji_occurrences.json), and official occurrence rows may store only level, source PDF, section, page, question reference, and observed kanji.
- Restricted or non-assignment lanes remain fenced. `tanos_frequency_method_notes` is active non-voting methodology evidence. `ask_hajimete_jlpt_kanji` is active only for pinned, reviewed N1, N2, N3, and separately verified N5 manual-citation rows with exact assignment proof; N4 remains unsupported until exact permitted source access is verified. `joyo_grade` is background metadata only, `bccwj_frequency` is frequency sanity only, `kanji_alive` is learner/background metadata only, and `jpdb` is a possible frequency sanity lane pending governed source-use review; none are assignment truth. `kanshudo` is blocked until source-use terms and a governed use path are approved. `wanikani` is blocked until source-use/API/export terms and a governed use path are approved. `jlptsensei` is a secondary non-Japanese manual-citation signal only after Japanese-published evidence is no longer the dominant blocker; do not scrape, copy, or republish lists. `try_jlpt_textbook` is blocked unless exact per-kanji assignment proof is found. `japanese_textbook_consensus` is an active derived non-voting summary computed from the individual textbook lanes.
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `382` high-confidence rows, `51` standard-confidence rows, `18` disputed rows, `1761` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1673` insufficient independent source rows, `1402` insufficient independent evidence-lineage rows, `1409` missing Japanese-published source rows, `18` disputed consensus rows, and `197` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1818` rows needing governed source review: `1409` missing Japanese-published source, `186` contract/consensus mismatch, `119` insufficient independent sources, `86` weak evidence, and `18` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `314` Shin Kanzen reviewed rows plus `303` `source_access_gap` rows (`617` resolved), and `442` Sou Matome reviewed rows plus `451` `source_access_gap` rows (`893` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
