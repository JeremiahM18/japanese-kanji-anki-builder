...
- Restricted or non-assignment lanes remain fenced. `tanos_frequency_method_notes` is active non-voting methodology evidence. `ask_hajimete_jlpt_kanji` is active only for pinned, reviewed N1-N3 manual-citation rows with exact assignment proof. `jlptsensei` and `try_jlpt_textbook` are registered restricted manual-citation lanes but remain inactive until reviewed rows, pinned source-input integrity, and explicit activation are in place; `try_jlpt_textbook` remains a level-study lane pending exact kanji-assignment-surface confirmation. `japanese_textbook_consensus` is an active derived non-voting summary computed from the individual textbook lanes. `kanshudo` and `wanikani` are blocked from consensus until a governed use path exists; `jpdb` is frequency-sanity only. Planned legal/free sanity lanes include BCCWJ frequency sanity checks and Kanji Alive background metadata.
