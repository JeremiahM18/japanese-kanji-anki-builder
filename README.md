...
| Surface | Current state | Main gates |
| --- | --- | --- |
| N5 word | `308/646` strict word Obsidian-certified. The 338 remaining current N5 word v2 dictionary-common-pool Silver additions are generated-ready with word audio, pitch, required back-side fields, examples, reading breakdowns, support labels, and readiness, but they have not completed Gold/Sapphire/Platinum/Obsidian. No N5 source-only phrase exclusions remain in the tracked word contract. Live readiness is `ready_with_deferred_variants`; reading coverage is `70.6%` (`243/344`). Release artifact QA, accessibility, and listening checks are still required before release-ready product claims. | `deck:words:obsidian:certify-status -- --levels=5`, `deck:words:ready -- --levels=5` |
