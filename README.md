...
| `ask_hajimete_jlpt_kanji` | [ASK Hajimete JLPT kanji books](https://ask-books.com/series/jlpt-kanji/) | Registered restricted manual-citation kanji textbook lane for exact N1-N3 source-access spikes; inactive/non-voting until reviewed rows are pinned and source activated |
...
- Current active assignment lanes are still evidence, not deck movement. `kanjidic2_legacy` has `1479` reviewed exact assignments and `0` current range rows; future old JLPT 2 rows stay N2/N3 range evidence when present. `tanos_legacy_direct` has `1478` reviewed N1/N4/N5 assignments. `tanos_estimated_split` has `734` lower-weight reviewed estimated N2/N3 assignments (`367` N2 and `367` N3) and must not settle taxonomy movement by itself. `shin_kanzen_master_kanji` has `288` active restricted manual-citation N4 assignments from the pinned ignored worksheet; `1608` worksheet rows are still pending and `316` checked rows are marked non-importing `source_access_gap` until fuller source access can provide exact source-level evidence. `nihongo_sou_matome_kanji` has `426` active restricted manual-citation assignments from the pinned ignored worksheet; `1786` worksheet rows are still pending. `official_jlpt_sample_workbooks` is active occurrence-only evidence backed by [templates/jlpt_official_kanji_occurrences.json](templates/jlpt_official_kanji_occurrences.json), and official occurrence rows may store only level, source PDF, section, page, question reference, and observed kanji.
- Restricted or non-assignment lanes remain fenced. `tanos_frequency_method_notes` is active non-voting methodology evidence. `jlptsensei`, `try_jlpt_textbook`, and `ask_hajimete_jlpt_kanji` are registered restricted manual-citation lanes but remain inactive until reviewed rows, pinned source-input integrity, and explicit activation are in place. `ask_hajimete_jlpt_kanji` is configured for N1-N3 source-access spikes only based on current official product surfaces; `try_jlpt_textbook` remains a level-study lane pending exact kanji-assignment-surface confirmation. `japanese_textbook_consensus` is an active derived non-voting summary computed from the individual textbook lanes. `kanshudo` and `wanikani` are blocked from consensus until a governed use path exists; `jpdb` is frequency-sanity only. Planned legal/free sanity lanes include BCCWJ frequency sanity checks and Kanji Alive background metadata.
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `268` high-confidence rows, `52` standard-confidence rows, `12` disputed rows, `1880` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1815` insufficient independent source rows, `1586` insufficient independent evidence-lineage rows, `1594` missing Japanese-published source rows, `12` disputed consensus rows, and `197` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1922` rows needing governed source review: `1594` missing Japanese-published source, `184` contract/consensus mismatch, `69` insufficient independent sources, `63` weak evidence, and `12` disputed consensus, with source-input progress at `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved) and `426` Sou Matome reviewed rows (`426` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
