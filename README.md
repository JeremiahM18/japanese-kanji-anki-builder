...
| N4 kanji | `212/212` Gold and Platinum. `180/212` are Obsidian; `32` remain in the square-zero Obsidian queue. | `deck:platinum:n4`, `deck:platinum:rereview-status -- --levels=5,4`, `deck:ready -- --levels=4` |
...
| N4 word | `667` canonical rows. Gold and word-level placement pass, but readiness is still `incomplete`. Platinum coverage is `96/667`; Obsidian is `0/667`; `571` rows still lack active current-standard structural entries. | `deck:words:platinum:n4`, `deck:words:completion:n4`, `deck:words:gap-plan:n4` |
...
Word Platinum uses `word-platinum-v3-evidence-lanes`. Current N5/N4 word Obsidian status is `8/954` certified, `375` Platinum entries needing Obsidian, and `571` blocked/failing rows due to incomplete N4 structural coverage. The word source-posture report currently marks `268/383` structurally current-standard word entries as single-source-family, with `word_source_independence_not_proven`.

### Cross-Product Gates

- `deck:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 kanji as `292/292` Platinum, `260/292` Obsidian, `32/292` needing Obsidian, and `0/292` blocked/failing.
- `deck:kanji:platinum:certify-status -- --levels=5,4` is the fail-closed kanji Obsidian gate. It currently fails on the `32` N4 kanji that still need Obsidian proof.
