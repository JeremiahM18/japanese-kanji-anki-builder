...
| N4 kanji | Golden-reviewed; current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `212/212`, and `30/212` now have explicit non-mechanical substantive rereview proof; the remaining `182` N4 kanji stay in the square-zero rereview queue |
...
- `deck:platinum:rereview-status -- --levels=5,4` is the conservative kanji rereview provenance report. Current N5/N4 output is `292/292` structural v3 gate pass, `110/292` explicit substantive current-standard rereview proven, `182/292` structural-only entries needing substantive rereview, and `0/292` blocked/failing. Its searchable limitation marker is `missing_substantive_current_standard_rereview_proof`.
