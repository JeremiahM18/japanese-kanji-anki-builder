...
All five JLPT levels are first-class product surfaces. N5/N4 kanji are Obsidian certified; N3 kanji structural Platinum proof is complete and Obsidian proof has started; N2/N1 are still represented below as generated, readiness, Gold/Silver, and backlog posture.
...
| N3 kanji | `341/341` generated, Gold, and current-standard structural Platinum; Obsidian is `120/341`, and `221` Platinum entries still need substantive Obsidian proof. | `deck:kanji:review-status`, `deck:ready -- --levels=3`, `deck:platinum:n3`, `deck:kanji:obsidian:rereview-status -- --levels=3` |
...
For migrated kanji proof levels (N5/N4/N3), switched kanji proof consumers read scoped canonical JSONL Obsidian proof from `templates/obsidian_proof_ledger/*.jsonl`; tracked review-set JSON no longer carries inline Obsidian proof objects for those levels. Kanji levels without scoped proof ledgers still fall back through the provider path until their own proof ledgers are deliberately migrated and gated.
...
- `deck:words:obsidian:rereview-status -- --levels=5,4` uses generated word rows as the denominator. N5 word `287/287` and N4 word `700/700` Obsidian remain separate from `987/987` structural Platinum.
...
- `deck:platinum:governance-gate` exercises real generated N5/N4 kanji and word rows when ignored local inputs are present. It currently passes with governance warnings for word single-source-family posture, bulk-template or missing card-specific revalidation summaries, marker-only example-quality automation, and zero active verification limitations.
...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `498` reviewed assignments, `417` source_access_gap rows, and `1297` pending rows; continue targeted review only where exact assignment proof is available |
...
Benchmark budget commands are manual/local performance guardrails, not GitHub Actions CI gates. `data:benchmark:jlpt:sources:gate`, `bench:obsidian-proof-etl:gate`, `bench:build:gate`, and `bench:build:cold-apkg:gate` are manual/local performance guardrails for source-evidence, Obsidian proof ETL, hot-cache build, and cold native APKG package-performance changes. `bench:build:gate` and `bench:build:cold-apkg:gate` require a ready local workspace and write benchmark output under the configured benchmark output directory. `bench:build:cold-apkg:gate` clears the generated APKG cache and gates the package phase only; the hot build gate owns total/export/media-sync budgets. `perf:memory:matrix` is a CI-safe metadata audit, not a timing budget gate.
