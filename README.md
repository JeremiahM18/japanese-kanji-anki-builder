...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `366` reviewed assignments from the pinned ignored worksheet; `1846` worksheet rows remain pending |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `268` high-confidence rows, `52` standard-confidence rows, `9` disputed rows, `1883` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1816` insufficient independent source rows, `1645` insufficient independent evidence-lineage rows, `1653` missing Japanese-published source rows, `9` disputed consensus rows, and `174` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1922` rows needing governed source review: `1653` missing Japanese-published source, `160` contract/consensus mismatch, `63` weak evidence, `37` insufficient independent sources, and `9` disputed consensus, with source-input progress at `288` Shin Kanzen reviewed rows plus `316` `source_access_gap` rows (`604` resolved) and `366` Sou Matome reviewed rows (`366` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
