...
| `nihongo_sou_matome_kanji` | [Nihongo Sou Matome textbooks](https://www.ask-books.com/jp/somatome/) | Active restricted manual-citation textbook lane with `441` reviewed assignments and `422` non-importing `source_access_gap` rows; `1349` worksheet rows remain pending until exact source-level evidence is reviewed |
...
- The current source-evidence audit is still expected to fail on taxonomy confidence, not source-use hygiene. The latest `npm run data:audit:jlpt:sources -- --governance-strict --limit=25` run reports governance passing and evidence-depth failing: `382` high-confidence rows, `51` standard-confidence rows, `18` disputed rows, `1761` weak-evidence rows, and `0` unknown rows. Issue counts are `0` missing evidence rows, `1673` insufficient independent source rows, `1403` insufficient independent evidence-lineage rows, `1410` missing Japanese-published source rows, `18` disputed consensus rows, and `196` current-contract/source-consensus mismatches; source-use, license/use evidence, illegal consensus use, and disallowed stored-assignment blockers are currently `0`. The all-level source-review batch worklist currently reports `1818` rows needing governed source review: `1410` missing Japanese-published source, `185` contract/consensus mismatch, `119` insufficient independent sources, `86` weak evidence, and `18` disputed consensus, with source-input progress at `208` ASK Hajimete reviewed rows (`208` resolved), `314` Shin Kanzen reviewed rows plus `303` `source_access_gap` rows (`617` resolved), and `441` Sou Matome reviewed rows plus `422` `source_access_gap` rows (`863` resolved). No deck movement, word movement, or readiness change should happen from JLPT level assumptions before taxonomy confidence is source-backed and reviewed.
