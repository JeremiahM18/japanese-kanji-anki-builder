...
| N5 word | Expanded to `287` canonical governed rows plus `20` tracked source-only phrase exclusions; current word-level placement, golden, tracked-source artifact, and automated readiness checks pass. Combined current and legacy platinum history covers `287/287` only with `--allow-legacy-standard`; current-standard word platinum is `216/287` under `word-platinum-v2-limitation-aware`, with `71` legacy/unversioned active entries still requiring revalidation. N5 word release still needs current-standard platinum revalidation, manual import QA, accessibility, and listening checks before release-ready status. |
...
- N5 word golden review passes against the current `287` generated rows. N5 combined current and legacy word platinum passes only with `--allow-legacy-standard`; the default current-standard word platinum command fails at `216/287` current-standard entries and `71` legacy/unversioned active entries remaining. N4 word golden review currently fails at `660/667` generated rows because several tracked breakdown expectations drift from the generated surface; N4 platinum has not started. Golden/platinum commands are still separate from APKG import QA, manual card QA, accessibility checks, and listening review.
...
- Current word platinum standard is `word-platinum-v2-limitation-aware`. Active word platinum entries without `reviewStandard`, `revalidatedAt`, `revalidationSummary`, `notesIncludes`, and `current-standard-review` evidence are reported as legacy/unversioned and do not pass the default platinum commands. Use `--allow-legacy-standard` only for historical inspection while revalidating.
...
- Kanji platinum entries created before `kanji-platinum-v2-limitation-aware`, and word platinum entries created before `word-platinum-v2-limitation-aware`, are legacy/unversioned history, not trusted release coverage, until they are revalidated and pass the default platinum command.
