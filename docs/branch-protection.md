...
Changes that affect JLPT taxonomy, starter curation, golden review placement, source-evidence inputs, or deck-membership logic also require `npm run data:audit:jlpt`, the read-only `npm run data:audit:jlpt:sources -- --limit=25` transparency audit, and the relevant strict `npm run data:audit:jlpt:source-inputs -- --source=<source-id> --strict` preflight before merge. Source-evidence input imports must also dry-run `npm run data:import:jlpt:source-input -- --source=<source-id>` before any `--write` update to the tracked manifest.
