...
| `npm run deck:words:vocab-expansion -- --levels=5 --source=common-pool --strict --limit=80` | Continue the same extra expansion lane with the `DICTIONARY COMMON POOL` after reading, current-source, and reviewed extra-source-family work are exhausted; default editorial mode orders the shortlist by transparent learner-utility score before capping the human review queue at `200` rows per selected level while keeping raw and eligible pool counts audit-visible; the score includes everyday usefulness, concrete/common domain fit, target-kanji reinforcement value, duplicate or near-duplicate safety, specialized/proper-noun penalty signals, exampleability, and pitch/audio/media readiness, and remains an ordering signal only, not card approval; rows come from pinned JMdict dictionary/commonness inputs, exclude exact governed/excluded duplicates and kana-only rows by default, do not automatically deprioritize outside-JLPT or higher-level support kanji when a level-relevant anchor exists, stay labeled `EXTRA SOURCE FAMILY`, `DICTIONARY COMMON POOL`, and `Source level claim unverified`, and remain pre-trust triage work rather than Silver promotions |
| `npm run deck:words:vocab-expansion -- --levels=5 --source=common-pool --common-pool-mode=raw --strict --limit=80` | Audit the full dictionary common-pool denominator without the default `200`-row editorial shortlist; use `--common-pool-mode=raw` for evidence accounting, and do not treat raw mode as the learner-facing review queue |
