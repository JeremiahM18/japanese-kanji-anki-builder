...
| `npm run deck:words:vocab-expansion -- --levels=5,4,3,2,1` | Run the same selector in vocabulary-level placement mode for post-reading-expansion common source-listed vocabulary; the queue is inactive until the level's reading-gap expansion signal is exhausted, top-level `move_candidate` stays authoritative in every placement mode, standalone one-kanji written forms stay queue-eligible, and ungoverned moved rows are surfaced in their target level's queue |
