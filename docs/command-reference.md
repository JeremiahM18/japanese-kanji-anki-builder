...
| `npm run data:build:kanji-field-source-contract -- --level=<level>` | Build a tracked per-level kanji card-field source contract from current-standard Platinum Japanese-source evidence without reading ignored `data/` inputs |
