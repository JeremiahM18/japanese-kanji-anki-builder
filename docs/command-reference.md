...
| `npm run deck:words:vocab-expansion -- --levels=5,4,3,2,1` | Run the same selector in vocabulary-level placement mode for post-reading-expansion common source-listed vocabulary; fallback/free-source rows stay closed until readings and the current selector are exhausted, extra rows must remain labeled `Source level claim unverified`, top-level `move_candidate` stays authoritative in every placement mode, standalone one-kanji written forms stay queue-eligible, and ungoverned moved rows are surfaced in their target level's queue |
