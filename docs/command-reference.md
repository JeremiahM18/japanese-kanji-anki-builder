...
| `npm run deck:ops -- --deck=word --lane=sapphire --level=5` | Print a read-only operations plan for the selected deck/program-lane selector/level: current git/change-risk posture, visible lower-lane backlog from closeout counts, exact next legal commands, focused verification, full merge gate, safe parallelism, serial write roots, architecture prerequisites, and fail-closed rules; alias: `npm run deck:lane-status`. The true program lane order is Discover, Silver, Gold, Sapphire, Platinum, Obsidian; support selectors such as NLP, source governance, media/audio, release, and ops are work areas, not certification lanes. This command does not run lane gates, write proof/source/templates, certify cards, shrink denominators, or hide expected backlog |
...
| `npm run verify:focused -- --deck=word --lane=sapphire --level=5` | Print a read-only focused verification plan for the selected deck/program-lane selector/level: live git posture, changed-file risk, exact focused commands, mapped focused tests, changed-file-specific commands, full merge gate, source-of-truth docs, and hard boundaries. Support selectors remain work areas, not certification lanes. This command does not run gates, write proof/source/templates, certify cards, replace the full merge gate, shrink denominators, or hide expected backlog |
