...
| `npm run deck:words:expansion-status -- --levels=5,4,3,2,1` | Show the vocabulary-level word expansion work order for each selected level: reading fast promotions, reading editorial research, current selector ready/triage/move work, deferred/blocked backlog, and whether the extra expansion lane is ready; reviewed extra/free source-family selectors run before the dictionary common pool, and the common pool opens in the same lane instead of sending reviewers into another source hunt; read-only and never removes required source labels |
