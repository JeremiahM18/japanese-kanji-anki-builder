...
| `npm run deck:ops -- --deck=word --lane=nlp --level=5` | Print a read-only lane operations plan for the selected deck/lane/level: current git/change-risk posture, visible lower-lane backlog from closeout counts, exact next legal commands, focused verification, full merge gate, safe parallelism, serial write roots, architecture prerequisites, and fail-closed rules; alias: `npm run deck:lane-status`; this command does not run lane gates, write proof/source/templates, certify cards, shrink denominators, or hide expected backlog |
...
| `npm run verify:focused -- --deck=word --lane=nlp --level=5` | Print a read-only focused verification plan for the selected deck/lane/level: live git posture, changed-file risk, exact focused lane commands, mapped focused tests, changed-file-specific commands, full merge gate, source-of-truth docs, and hard boundaries; this command does not run gates, write proof/source/templates, certify cards, replace the full merge gate, shrink denominators, or hide expected backlog |
