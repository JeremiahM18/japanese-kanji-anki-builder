...
| `npm run deck:words:vocab-expansion -- --levels=5 --source=common-pool --strict --limit=80` | Continue the same extra expansion lane with the `DICTIONARY COMMON POOL` after reading, current-source, and reviewed extra-source-family work are exhausted; rows come from pinned JMdict dictionary/commonness inputs, exclude exact governed/excluded duplicates and kana-only rows by default, stay labeled `EXTRA SOURCE FAMILY`, `DICTIONARY COMMON POOL`, and `Source level claim unverified`, and remain pre-trust triage work rather than Silver promotions |
