...
| `npm run deck:words:vocab-expansion -- --levels=5 --source=common-pool --strict --limit=80` | Continue the same extra expansion lane with the `DICTIONARY COMMON POOL` after reading, current-source, and reviewed extra-source-family work are exhausted; default editorial mode caps the human review queue at `200` rows per selected level while keeping raw and eligible pool counts audit-visible, rows come from pinned JMdict dictionary/commonness inputs, exclude exact governed/excluded duplicates and kana-only rows by default, do not automatically deprioritize outside-JLPT or higher-level support kanji when a level-relevant anchor exists, stay labeled `EXTRA SOURCE FAMILY`, `DICTIONARY COMMON POOL`, and `Source level claim unverified`, and remain pre-trust triage work rather than Silver promotions |
