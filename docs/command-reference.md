...
| `npm run data:migrate:word-sapphire-slim` | Dry-run a count-preserving word Sapphire manifest slimming migration that removes prior-lane copied fields only after unique generated, Gold, and Sapphire bindings resolve; `--write` updates tracked word Sapphire manifests only after the dry-run is clean and does not create Platinum, Obsidian proof, card approval, or denominator changes |
