...
| `npm run data:obsidian:proof:append -- --events=out/obsidian-proof/drafts/<batch>.jsonl` | Dry-run append of complete Obsidian proof-event drafts to the canonical JSONL ledger. Rerun with `--write` only after the dry-run is clean; validates schema, card binding, tracked review-set targets, duplicate proof ids, duplicate proof targets, canonical ledger path, and post-write reconciliation |
