...
| `npm run deck:words:expansion-status -- --levels=5,4,3,2,1` | Show the vocabulary-level word expansion work order for each selected level: reading fast promotions, reading editorial research, current selector ready/triage/move work, deferred/blocked backlog, and whether the extra free/permitted source-family lane is ready; if no actionable extra free source is registered, the report says so instead of sending reviewers into another source hunt; read-only and never removes required source labels |
