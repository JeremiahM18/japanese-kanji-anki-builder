...
| `npm run product:release-qa:evidence` | Validate the release-specific QA evidence packet for APKG import, managed media, Anki import, mobile, accessibility, listening QA, source-governance posture, and known blockers |
...
| `npm run deck:platinum:n1` | Run the N1 kanji Platinum gate; the trusted N1 Platinum scope is currently `256/1230`, so the full-level gate fails closed until all generated N1 rows have fresh governed current-standard entries |
