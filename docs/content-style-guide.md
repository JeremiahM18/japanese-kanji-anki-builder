...
- A word with no current-level anchor may ship in an easier/higher-numbered vocabulary deck only through the vocabulary-level expansion path: exact active source-list identity for that JLPT level, dictionary verification, commonness support, no duplicate governed identity, explicit triage acceptance, and `levelPlacement.mode: "vocabulary-level"` with a reason explaining the support-kanji labels.
...
- Legacy `move_candidate` triage means the row does not fit the kanji-anchor expansion view. If the same row is intentionally accepted as common N-level vocabulary, keep the anchor decision and add `placementDecisions["vocabulary-level"]` instead of overwriting the anchor evidence.
