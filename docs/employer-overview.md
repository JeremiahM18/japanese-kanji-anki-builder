...
| Governed surfaces | Current generated denominators cover `2212` core kanji rows and `2463` word rows across JLPT N5-N1. | The system is a multi-surface data product, not a single static deck. |
| Locked certification scope | The current scoped lock certifies `982` kanji rows and `1000` word rows through Obsidian. | Release claims are explicit, bounded, and separate from ongoing work. |
| Proof model | Canonical JSONL proof ledger validates with `1982` events across 6 ledger files. | Certification evidence is tracked, queryable, and replayable. |
...
| Words | `2463/2463` across N5-N1 | `1000/2463` | The lock covers the 300-row N5 and 700-row N4 Obsidian-certified subsets. Current N5/N4 word v2 Silver additions are generated-ready but not Obsidian-certified. |
