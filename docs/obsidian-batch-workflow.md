...
- `deck:words:obsidian:rereview-status` and `deck:words:obsidian:certify-status` are the switched word status consumers. For migrated N5/N4 word proof they default to `ledger-if-available` and can be audited with `--proof-provider=inline`, `--proof-provider=ledger`, or `--proof-provider=ledger-if-available`. Word Platinum batch and level gates, word governance inputs, and inline word proof removal remain later one-consumer-at-a-time lanes.
