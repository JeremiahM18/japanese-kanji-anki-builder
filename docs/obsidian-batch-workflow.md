...
- `deck:words:obsidian:rereview-status`, `deck:words:obsidian:certify-status`, `deck:words:platinum:batch`, and `deck:words:platinum:n<level>` are the switched word consumers. For migrated N5/N4 word proof they default to `ledger-if-available` and can be audited with `--proof-provider=inline`, `--proof-provider=ledger`, or `--proof-provider=ledger-if-available`. Word governance inputs and inline word proof removal remain later one-consumer-at-a-time lanes.
