...
Gold regression means the exported card surface is protected against drift. Sapphire means the live generated card passes current field-bound evidence and quality requirements. Platinum means the stronger future content-certification lane once its schema exists. Obsidian means explicit non-mechanical current-version rereview proof exists for the live card.
...
| Sapphire | Gate current structural/card-quality requirements. | The card passes the governed product contract against live generated rows, including field-bound source evidence, explicit quality gates, media identity, limitation handling, and reviewer judgment. Core kanji and words use native Sapphire commands; additional surfaces still use compatibility names until migrated. | Future Platinum content certification, Gold regression, Obsidian proof, release readiness, or manual QA. |
...
- Do not use Sapphire or compatibility Platinum coverage as a substitute for Gold regression, future Platinum content certification, or Obsidian proof.
...
Legacy N5/N4 word Platinum compatibility level gates remain only for downstream proof-provider compatibility while consumers migrate. The boundary remains: a passing Sapphire or legacy compatibility gate is not future Platinum content certification, new Obsidian certification, or release readiness.
