...
- **Platinum Candidate**: current-standard structural gate passes, including evidence lanes, field bindings, governed source posture, media identity, and required quality gates. This is not substantive certification by itself.
- **Platinum**: the card has explicit non-mechanical current-version rereview provenance after the reviewer actually performs the substantive review.
- **Obsidian**: reserved for a future fluent/native-level language audit. Do not claim Obsidian for current best-effort non-native review.
...
Only current-standard `platinum` and `fixed_then_platinum` manifest entries count as **Platinum Candidate** structural coverage until they also carry explicit substantive rereview provenance. `deferred` and `removed` entries must not appear in generated exports. `needs_revalidation` is allowed as non-certifying history and still counts as missing Platinum Candidate coverage. `needs_review` always fails the candidate gate.
...
`deck:platinum:rereview-status` is a read-only kanji rereview provenance report. It separates **Platinum Candidate** structural pass from **Platinum** current-version certification proof. It must not infer proof from `revalidatedAt` or lane-valid `current-standard-review` text alone; missing proof is reported with the searchable marker `missing_substantive_current_standard_rereview_proof`. Use it to classify before rereviewing rather than globally blocking structurally valid cards or silently overclaiming human rereview provenance.

The generated deck-row count is the certification denominator. Platinum Candidate counts are diagnostics only; they must not shrink the square-zero rereview queue.
...
Core kanji and word Platinum Candidate commands intentionally fail for an empty manifest. Do not use Gold regression coverage as a substitute for Platinum Candidate review, and do not use Platinum Candidate coverage as a substitute for Platinum certification.
