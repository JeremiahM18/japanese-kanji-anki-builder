...
The older `deck:platinum:rereview-status`, `deck:words:platinum:rereview-status`, `deck:kanji:platinum:certify-status`, and `deck:words:platinum:certify-status` names remain compatibility aliases only. The kanji rereview alias now defaults to the scoped `ledger-if-available` proof provider; use the Obsidian names in new docs, release checklists, handoff prompts, and governance reports.
