...
`product:artifacts:kanji:n5:preflight` reports whether tracked templates are sufficient to certify fresh N5 kanji TSV generation without ignored local `data/` inputs. It currently reports that certification is blocked because rich kanji source provenance is not a tracked release contract yet. Component/radical source data is tracked in `templates/kanji_component_contract.json`; on/kun reading reference data is tracked in `templates/kanji_reading_reference_contract.json`.
