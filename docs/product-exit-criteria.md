...
- Kanji platinum rereview provenance: `deck:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `292/292` structural v3 gate pass, `170/292` substantive current-standard review proven, `122/292` structural-only entries needing substantive rereview, and `0/292` blocked/failing. The missing-proof marker is `missing_substantive_current_standard_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
