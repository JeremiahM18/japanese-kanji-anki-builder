...
- N5 word: strict non-human governed native/fluent-quality word Obsidian content certification covers `300/346` current generated rows. The 46 current N5 word v2 dictionary-common-pool Silver additions have generated card surface, required back-side fields, word audio, pitch accent, examples, reading breakdowns, support labels, and readiness, but they have not entered Gold/Sapphire/Platinum/Obsidian. Word-level placement, tracked-source artifact, automated readiness, and lower-lane prerequisites pass for the certified subset; `20` tracked source-only phrase exclusions remain outside the generated denominator; current live readiness is `ready_with_deferred_variants`, reading coverage is `234/344` (`68.0%`), and release artifact QA, accessibility, and listening checks are still required before release-ready product claims
...
- Word Obsidian provenance: `deck:words:obsidian:rereview-status -- --levels=5,4` currently reports N5/N4 combined `1000/1065` Obsidian certified for current generated rows; the 46 current N5 word v2 Silver rows and 19 current N4 word v2 Silver rows need downstream proof. Lower-lane prerequisites are complete for the certified subset. The generated-row denominator is the square-zero Obsidian denominator; for N5 word that now means `300/346` Obsidian certified and for N4 word that now means `700/719`, not `1000/1065` because a lower lane is complete. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt`, lane-valid `current-standard-review` text, and loose textual markers are not treated as standalone proof of substantive post-v3 Obsidian rereview. Word Obsidian proof must be structured, exact word-reading-card-bound, and backed by a full evidence checklist plus actual example-sentence quality review evidence.
- Word Obsidian certification gate: `deck:words:obsidian:certify-status -- --levels=5,4` is expected to fail for the current full N5/N4 generated denominator until the 46 current N5 word v2 Silver rows and 19 current N4 word v2 Silver rows complete Gold, Sapphire, Platinum, and Obsidian. It fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with field, expected, actual, evidence lane, and reviewer action.
