...
- N4 word: current word Obsidian v2.5 non-human governed native/fluent-quality content certification covers `192/1034` current generated rows. Gold, current-standard Sapphire, and current-standard Platinum are complete at `1034/1034`; the Platinum missing-current-standard queue is empty. The `700` older N4 Obsidian proof targets remain legacy history, not current v2.5 certification; `842` Platinum-passing rows still need separate substantive Obsidian v2.5 proof. Current live readiness is incomplete because reading-gap override alignment is not clear; reading coverage is `581/755` (`77.0%`), the active triage backlog is clear, and the remaining coverage items are deferred variants or low learner-value gaps. N4 word still needs release artifact QA, accessibility, and listening checks before release-ready product claims.
- Word Obsidian provenance: `deck:words:obsidian:rereview-status -- --levels=5,4` currently reports N5/N4 combined `780/1622` current word Obsidian v2.5-certified for current generated rows, with `842` Platinum-passing N4 rows needing Obsidian v2.5 and `0` N4 rows blocked on Platinum. N4 Gold, current-standard Sapphire, and current-standard Platinum are complete at `1034/1034`. Legacy word Obsidian proof history remains audit-visible at `1118` N5/N4 targets and `1898` raw ledger events; `610` older same-target proof events are superseded by current v2.5 proof. The generated-row denominator is the square-zero Obsidian denominator; legacy proof history, `revalidatedAt`, lane-valid `current-standard-review` text, and loose textual markers are not treated as standalone proof of current Obsidian v2.5 rereview. Word Obsidian proof must be structured, exact word-reading-card-bound, backed by a full evidence checklist plus actual example-sentence quality review evidence, and backed by exact example-sentence audio proof.
- Word Obsidian certification gate: `deck:words:obsidian:certify-status -- --levels=5,4` is expected to fail for the current full N5/N4 generated denominator until the remaining `842` N4 Platinum-passing rows complete Obsidian v2.5 proof. N5 alone now passes current v2.5 certification. The gate fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with field, expected, actual, evidence lane, and reviewer action.
...
npm run product:artifacts:kanji:release-qa # production/GA only; remains strict
...
`product:artifacts:kanji:all` runs the tracked-source kanji TSV artifact gate across N5 through N1. Current expected posture is N5/N4/N3 passing and N2/N1 blocked on missing governed card-field source contracts. `product:artifacts:kanji:release-qa` remains the strict production/GA gate and blocks until actual APKG import, managed media, mobile, screen-reader, listening, and visual QA are recorded. It is intentionally not weakened for a preview. Run accessibility review with the exact candidate `--run-id` and per-deck `--levels`. `product:release-qa:evidence` validates packet version 3, package version/tag/full-commit/release-class binding, one exact APKG per scoped deck kind with local path plus release basename, canonical levels, note/card/media counts, byte size and SHA-256, commit-bound automated/artifact evidence, release policy, source posture, and empty known blockers. Production allows only passed artifact QA; an automation-reviewed preview permits only explicit `PROD-REL-001` accepted-risk evidence and the required prerelease label. `product:release-qa:apkg-inspect -- --require-golden` independently validates APKG ZIP/media/SQLite/deck/note/card/field/reference structure and every packaged learner-facing row against tracked full-level Golden manifests, but it does not simulate Anki or human perception.
...
`product:readiness:n5` is the full local-data N5 product checkpoint. It runs the JLPT kanji audit, JLPT word audit, governed audio provenance audit, tracked-source N5 word TSV artifact checkpoint, tracked-source N5 kanji TSV artifact checkpoint, N5 word-level placement audit, N5 kanji Gold regression, and N5 word Gold regression. `-- --tracked-only` is the clean-hosted companion: it derives a minimal runtime level map from the tracked contract inside a disposable workspace and runs only the six reproducible tracked-input checks. It explicitly excludes local media provenance and regenerated local Gold; the tagged release workflow checks Golden expectations inside the exact APKGs instead. The word placement audit distinguishes rows without a current-level anchor from later all-easier-kanji placements without explicit learner-fit rationale. Neither mode certifies Platinum or Obsidian by itself, performs human/device QA, or proves hosted release trust.
