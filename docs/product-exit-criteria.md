...
- N5 word: strict non-human governed native/fluent-quality word Obsidian content certification covers `308/588` current generated rows. N5 Gold and Sapphire are complete at `588/588`, and current-standard Platinum is `407/588`; the 280 current N5 word rows without Obsidian proof have generated card surface, required back-side fields, word audio, pitch accent, examples, reading breakdowns, support labels, readiness, Gold regression, and Sapphire coverage, but they have not completed Obsidian proof, and 181 still need current-standard Platinum before Obsidian. Word-level placement, tracked-source artifact, automated readiness, and lower-lane prerequisites pass for the certified subset; no N5 source-only phrase exclusions remain in the tracked word contract; current live readiness is `ready_with_deferred_variants`, reading coverage is `245/344` (`71.2%`), and release artifact QA, accessibility, and listening checks are still required before release-ready product claims
...
- Word Obsidian provenance: `deck:words:obsidian:rereview-status -- --levels=5,4` currently reports N5/N4 combined `1008/1307` Obsidian certified for current generated rows; the 280 current N5 word v2 rows without Obsidian proof and 19 current N4 word v2 Silver rows need downstream proof. Lower-lane prerequisites are complete for the certified subset. The generated-row denominator is the square-zero Obsidian denominator; for N5 word that now means `308/588` Obsidian certified and for N4 word that now means `700/719`, not `1008/1307` because a lower lane is complete. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt`, lane-valid `current-standard-review` text, and loose textual markers are not treated as standalone proof of substantive post-v3 Obsidian rereview. Word Obsidian proof must be structured, exact word-reading-card-bound, and backed by a full evidence checklist plus actual example-sentence quality review evidence.
- Word Obsidian certification gate: `deck:words:obsidian:certify-status -- --levels=5,4` is expected to fail for the current full N5/N4 generated denominator until the 280 current N5 word v2 rows without Obsidian proof complete Obsidian, including 181 that still need current-standard Platinum first, and the 19 current N4 word v2 Silver rows complete Gold, Sapphire, Platinum, and Obsidian. It fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with field, expected, actual, evidence lane, and reviewer action.
