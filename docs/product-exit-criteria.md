...
- Word Obsidian provenance: `deck:words:obsidian:rereview-status -- --levels=5,4` currently reports N5/N4 combined `1000/1265` Obsidian certified for current generated rows; the 246 current N5 word v2 Silver rows and 19 current N4 word v2 Silver rows need downstream proof. Lower-lane prerequisites are complete for the certified subset. The generated-row denominator is the square-zero Obsidian denominator; for N5 word that now means `300/546` Obsidian certified and for N4 word that now means `700/719`, not `1000/1265` because a lower lane is complete. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt`, lane-valid `current-standard-review` text, and loose textual markers are not treated as standalone proof of substantive post-v3 Obsidian rereview. Word Obsidian proof must be structured, exact word-reading-card-bound, and backed by a full evidence checklist plus actual example-sentence quality review evidence.
- Word Obsidian certification gate: `deck:words:obsidian:certify-status -- --levels=5,4` is expected to fail for the current full N5/N4 generated denominator until the 246 current N5 word v2 Silver rows and 19 current N4 word v2 Silver rows complete Gold, Sapphire, Platinum, and Obsidian. It fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with field, expected, actual, evidence lane, and reviewer action.
