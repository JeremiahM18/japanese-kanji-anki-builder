...
- N4 word: expanded to `683` governed rows after two routed N5-source move batches. Current word-level placement passes and the generated card surface builds at `683/683`, with required back-side fields, word audio, pitch accent, examples, reading breakdowns, and support labels complete. Gold regression and current-standard Platinum structural coverage still cover `667/683`; the 16 newly routed rows are Silver only until explicit Gold and Platinum review entries are added. Current live completion reports `ready_with_deferred_variants`, expansion signals are exhausted, reading coverage is `578/755` (`76.6%`), and only deferred variants remain. `0/683` generated N4 word rows are Obsidian certified, so all N4 word rows remain in the square-zero Obsidian queue and N4 remains blocked before release-ready claims until Gold, Platinum, Obsidian, import QA, accessibility, and listening checks are complete.
- Word Obsidian provenance: `deck:words:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `970` generated active word rows, `954/970` Platinum structural pass, `8/970` Obsidian certified, `946` Platinum entries needing Obsidian, and `16/970` blocked/failing rows. The generated-row denominator is the square-zero Obsidian denominator; for N4 word that means `0/683` Obsidian certified, not `667/683` just because Platinum structural coverage exists. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
...
- Platinum governance gate: `deck:platinum:governance-gate` currently passes against local real generated N5/N4 rows with an `allowed_incomplete_word_platinum_level` warning for the 16 newly routed N4 Silver rows, plus warnings for missing Obsidian proof, word single-source-family posture, bulk-template or missing card-specific revalidation summaries, marker-only example-quality automation, and zero active verification limitations. This gate does not edit cards and does not prove word Obsidian certification or release readiness.
