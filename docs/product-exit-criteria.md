...
- Kanji Obsidian provenance: `deck:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `292/292` Platinum pass, `170/292` Obsidian certified, `122/292` Platinum entries needing Obsidian, and `0/292` blocked/failing. The missing-proof marker is `missing_substantive_current_standard_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
- Kanji Obsidian certification gate: `deck:kanji:platinum:certify-status -- --levels=5,4` currently fails because `122` N4 Platinum entries still need Obsidian proof. It fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with field, expected, actual, evidence lane, and reviewer action. Current language-review scope is `best_effort_non_native_review`; automation cannot fully prove natural Japanese or pedagogy without later fluent/native audit provenance.
