...
- N5 word: expanded to `287` canonical governed rows plus `20` tracked source-only phrase exclusions; current word-level placement, Gold, tracked-source artifact, automated readiness, Platinum, and Obsidian checks pass at `287/287`, with `0/287` remaining in the square-zero Obsidian queue; manual import QA, accessibility, and listening checks are still required before release-ready status
- N4 word: expanded to `700` governed rows after finishing the routed N5-source target-N4 move workload. Current word-level placement passes and the generated card surface builds at `700/700`, with required back-side fields, word audio, pitch accent, examples, reading breakdowns, and support labels complete. Gold regression now covers `700/700`; current-standard Platinum structural coverage still covers `667/700`; the 33 routed rows are Gold-reviewed but still awaiting explicit Platinum review entries. Current live readiness is `incomplete`, reading coverage is `573/755` (`75.9%`), and the reading backlog still includes review-needed/actionable items. `0/700` generated N4 word rows are Obsidian certified, so all N4 word rows remain in the square-zero Obsidian queue and N4 remains blocked before release-ready claims until Platinum, Obsidian, import QA, accessibility, and listening checks are complete.
- Word Obsidian provenance: `deck:words:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `987` generated active word rows, `954/987` Platinum structural pass, `287/987` Obsidian certified, `667` Platinum entries needing Obsidian, and `33/987` blocked/failing rows. The generated-row denominator is the square-zero Obsidian denominator; for N4 word that means `0/700` Obsidian certified, not `667/700` just because Platinum structural coverage exists. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
...
- Platinum governance gate: `deck:platinum:governance-gate` currently passes against local real generated N5/N4 rows with an `allowed_incomplete_word_platinum_level` warning for the 33 newly routed N4 Gold-reviewed rows that still need Platinum entries, plus warnings for missing Obsidian proof, word single-source-family posture, bulk-template or missing card-specific revalidation summaries, marker-only example-quality automation, and zero active verification limitations. This gate does not edit cards and does not prove word Obsidian certification or release readiness.
