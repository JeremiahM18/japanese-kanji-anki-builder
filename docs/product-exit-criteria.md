...
- N4 word: expanded to `667` governed rows and current word-level placement, Gold regression, and current-standard Platinum structural coverage pass at `667/667`, but readiness remains incomplete. `0/667` generated N4 word rows are Obsidian certified, so all N4 word rows remain in the square-zero Obsidian queue. Current live checks report completion `incomplete`, and `deck:words:ready -- --levels=4 --require-no-active-triage` remains blocked before full Obsidian completion, import QA, accessibility, and listening checks
