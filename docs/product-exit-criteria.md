...
- N4 word: expanded to `667` governed rows and current word-level placement and Gold review pass, but readiness remains incomplete. Platinum word review exists for `192/667`, but `0/667` generated N4 word rows are Obsidian certified; those `192` Platinum entries need Obsidian review and the other `475` generated rows still lack active current-standard structural entries. Current live checks report completion `incomplete`, and `deck:words:ready -- --levels=4 --require-no-active-triage` remains blocked before full Obsidian completion, import QA, accessibility, and listening checks
- Word Obsidian provenance: `deck:words:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `954` generated active word rows, `479/954` Platinum pass, `8/954` Obsidian certified, `471` Platinum entries needing Obsidian, and `475/954` blocked/failing because N4 word Platinum coverage is incomplete. The generated-row denominator is the square-zero Obsidian denominator; for N4 word that means `0/667` Obsidian certified, not `0/192`. The missing-proof marker is `missing_substantive_current_standard_word_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
- Word platinum source posture: `deck:words:platinum:source-posture -- --levels=5,4` currently inspects the `479` structurally current-standard word entries only, reporting `115` with independent source families proven, `364` with a single source family, and `0` missing governed source evidence. Source-family posture is not the rereview selection pool and not substantive platinum proof. Single-source-family entries remain structurally governed but must not be described as independently corroborated; the searchable marker is `word_source_independence_not_proven`. Word source-claim origin independence is not evaluated until a word source-origin manifest exists; the searchable marker is `word_source_claim_origin_independence_not_evaluated`.
