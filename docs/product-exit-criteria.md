...
- N5 kanji: golden-reviewed and current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `80/80`, and `80/80` now have explicit non-mechanical substantive rereview proof
- N4 kanji: golden-reviewed and current local deck readiness passes with complete exported media and exact primary-reading audio; current v3 structural gates pass at `212/212` with `0` active verification limitations and `0` remaining core N4 structural gaps, and `30/212` now have explicit non-mechanical substantive rereview proof; the remaining `182` stay in the square-zero rereview queue
...
- Kanji platinum rereview provenance: `deck:platinum:rereview-status -- --levels=5,4` currently reports N5/N4 combined `292/292` structural v3 gate pass, `110/292` substantive current-standard review proven, `182/292` structural-only entries needing substantive rereview, and `0/292` blocked/failing. The missing-proof marker is `missing_substantive_current_standard_rereview_proof`; `revalidatedAt` and lane-valid `current-standard-review` text are not treated as standalone proof of substantive post-v3 human rereview.
