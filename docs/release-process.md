...
1. Confirm `npm test`, `npm run lint`, `npm run ci:smoke`, `npm run product:artifacts:n5` when N5 word ships, `npm run product:readiness:n5` when N5 ships, and `npm run release:gate` are green on the release commit. `product:artifacts:n5` is currently scoped to tracked-source N5 word TSV generation. `product:readiness:n5` is an automated N5 checkpoint, not a manual QA substitute. `release:gate` validates smoke-fixture artifacts and packaging contracts; it does not certify public product deck readiness.
