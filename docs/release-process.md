...
- Create Git tags as `v<package.json version>`, for example `v0.3.0-beta.5`.
...
5. Rerun owner-authenticated `npm run security:github-settings:auth`. Before the first successful tag, `artifact_attestation_verification_unproven` is the expected external blocker; after a successful tagged run it must pass before release-trust records close.
