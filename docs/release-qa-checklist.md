...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows and `700` strict Obsidian certifications, with lower-lane prerequisites complete, so `deck:words:obsidian:certify-status -- --levels=5,4` passes with `0` rows still needing Obsidian proof.
