...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows and `700` Platinum structural entries, so `448` strict Obsidian certifications still means only `448/700` proven, with `252` rows still needing Obsidian proof.
