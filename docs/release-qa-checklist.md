...
- For Tanos direct legacy updates, regenerate the ignored source TSV with `npm run data:normalize:tanos-jlpt-kanji`, pin its SHA-256, byte size, and row count, then dry-run `npm run data:import:jlpt:source-input -- --source=tanos_legacy_direct` before using `--write`. Keep Tanos N2/N3 in the separate inactive `tanos_estimated_split` lane until stronger governed evidence exists, and keep `tanos_frequency_method_notes` non-voting as method explanation only.
