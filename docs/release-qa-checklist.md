...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `675` generated rows, `667` Platinum structural entries, and `8` newly routed Silver rows, so `0` Obsidian certifications still means `0/675` proven.
