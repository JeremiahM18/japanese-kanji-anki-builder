...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `691` generated rows, `667` Platinum structural entries, and `24` newly routed Silver rows, so `0` Obsidian certifications still means `0/691` proven.
