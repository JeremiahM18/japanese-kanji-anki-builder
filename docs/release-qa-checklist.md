...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows, `700` Sapphire structural entries, `700` Platinum entries, and `700` strict Obsidian certifications, so `deck:words:obsidian:certify-status -- --levels=5,4` passes with `0` rows still needing Obsidian proof.
...
- Import each kanji level being shipped into Anki. Current ready local kanji levels are N5, N4, N3, N2, and N1. Current-standard native Sapphire coverage is complete for N5/N4/N3/N2; N1 is only partially trusted at `328/1230`, with `902` rows still requiring fresh actual card-data review before any version-1 structural lock. N1 deck readiness is mechanical/media readiness only until Sapphire and proof gates catch up.
