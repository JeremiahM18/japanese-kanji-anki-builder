...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows, `667` Platinum structural entries, and `33` newly routed Gold-reviewed rows awaiting Platinum, so `0` Obsidian certifications still means `0/700` proven.
