...
- Run the native Sapphire command for each version 1 structurally locked word level after its manifest is populated and current-standard revalidated, such as `npm run deck:words:sapphire:n5`.
...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows, `700` Sapphire card-quality entries, and `700` strict Obsidian certifications, so `deck:words:obsidian:certify-status -- --levels=5,4` passes with `0` rows still needing Obsidian proof.
- Run `npm run deck:words:legacy-platinum:source-posture -- --levels=<levels>` before claiming independent word-source corroboration. Single-source-family entries are structurally governed but are reported as `word_source_independence_not_proven`; word source-claim origin independence is reported as `word_source_claim_origin_independence_not_evaluated` until a word source-origin manifest exists. This legacy source-posture report is not native Platinum content certification.
- Run `npm run deck:legacy-platinum:governance-gate` in a local-data workspace before release claims that depend on current real generated N5/N4 rows. In clean CI without ignored `data/*` inputs, use its absence as a scope limitation rather than as proof that legacy structural governance or native Platinum content was validated.
