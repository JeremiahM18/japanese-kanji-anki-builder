...
- Run `npm run product:artifacts:kanji:n5:preflight` for an N5 kanji release. It currently reports tracked-source kanji TSV certification as blocked until rich kanji source provenance is a tracked release contract. Component/radical source data is tracked in `templates/kanji_component_contract.json`, and on/kun reading reference data is tracked in `templates/kanji_reading_reference_contract.json`.
