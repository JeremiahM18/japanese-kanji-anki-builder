...
- Run `npm run deck:kanji:platinum:certify-status -- --levels=<levels>` as the fail-closed kanji Obsidian gate. It fails on any `blocked_or_failing` or `needs_substantive_rereview` row and reports each failed card with the field, expected value, actual state, evidence lane, and reviewer action. Treat its `best_effort_non_native_review` language scope honestly unless fluent/native audit provenance exists.
...
- Treat generated deck rows as the certification denominator. For example, if N4 word has `667` generated rows and `96` Platinum entries, `0` Obsidian certifications means `0/667` proven, not `0/96`.
...
- Import each word level being shipped into Anki only after the current word-level placement audit, readiness audit, Gold review, applicable Platinum gate, and Obsidian status pass. N5 word currently passes placement, readiness, Gold, tracked-source artifact, and Platinum checks at `287/287` under `word-platinum-v3-evidence-lanes`; N4 word placement and Gold review pass at `667/667`, and Platinum coverage has started at `96/667`, but readiness remains incomplete with active triage and full Obsidian certification is not complete, so N4 remains blocked before manual QA/release.
