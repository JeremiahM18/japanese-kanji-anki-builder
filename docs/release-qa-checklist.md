...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows and `700` Platinum structural entries, so `136` strict Obsidian certifications still means only `136/700` proven.
