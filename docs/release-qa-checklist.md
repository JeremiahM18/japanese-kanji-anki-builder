...
- Treat generated deck rows as the certification denominator. For example, N4 word currently has `700` generated rows and `700` Platinum structural entries, so `424` strict Obsidian certifications still means only `424/700` proven, with `276` rows still needing Obsidian proof.
