...
| Obsidian | Proof-ledger native/fluent-quality content certification | Valid current-standard Platinum plus exact non-mechanical proof. | Explicit non-mechanical current-version native/fluent-quality rereview proof exists in the canonical proof path, binds to the exact live card identity, and is verified by the fail-closed Obsidian gate for the scoped version. | Human-reviewed provenance for the same standard, release readiness, packaged APKG/import/render/media/accessibility/listening QA, or source-taxonomy confidence unless separately recorded. |

## Content Certification And Release Artifact QA

Obsidian is the program's current non-human governed native/fluent-quality content-certification lane for a scoped version. It is not a shortcut from Gold, Sapphire, or Platinum; it requires the canonical proof ledger, exact live-card identity binding, structured rereview provenance, and a passing fail-closed Obsidian certification command. It must check natural Japanese, sense and translation fit, learner usefulness, level fit, reading/example quality, evidence, limitations, and release-quality content.

Release artifact QA is downstream of content certification. It proves the selected package imports, renders, plays media, remains accessible, and has no unresolved distribution blocker. It does not replace Obsidian or lower the native/fluent-quality content standard. When artifact QA exposes a content defect, the fix path is to reopen Sapphire, Platinum, and Obsidian for the affected cards.

Future human or native/fluent review is human-reviewed provenance for the same standard Obsidian already checks, not a different content standard.
