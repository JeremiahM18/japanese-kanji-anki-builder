...
| --- | --- | --- | --- | --- | --- | --- | --- |
| SEC-P0-001 | Critical | Mitigated | Repository owner | Hosted `main` must enforce the tracked branch-protection policy or required checks and review policy can be bypassed. | On 2026-09-03, the repository-owner authenticated audit verified that hosted main is protected and matches the tracked required checks and branch policy. Latest main CI and CodeQL conclusions are success. The first attempt used the other signed-in account and returned 403/404 on restricted endpoints; the command-scoped owner retry verified those endpoints without changing the active account. After PR #251 merged and its main CI and CodeQL runs succeeded, a fresh owner audit passed with no findings; the two dependency alerts are resolved as recorded below. | Keep the hosted audit in security verification; rerun after workflow names, required checks, branch policy, or repository ownership changes. | 2026-10-03 |
| SEC-P0-002 | High | Mitigated | Repository owner | Private vulnerability reporting must remain enabled for confidential intake. | On 2026-09-03, the repository-owner authenticated audit verified private vulnerability reporting, secret scanning, and push protection enabled, readable security settings, and zero open secret-scanning alerts. The post-merge owner audit passed with no findings after the two dependency alerts were resolved as recorded below. | Keep private vulnerability reporting enabled or record an approved private intake alternative in [../SECURITY.md](../SECURITY.md). | 2026-10-03 |
| SEC-P0-003 | High | Mitigated | Repository owner | Dependabot alert state and Dependency Graph must remain visible so open dependency alerts cannot hide from release-trust evidence. | On 2026-09-03, protected squash merge [PR #251](https://github.com/JeremiahM18/japanese-kanji-anki-builder/pull/251) landed at main commit `9737c46ec26ef63277ed6db2333b27449d4cdca1`. GitHub marked Dependabot alerts 6 (qs) and 7 (@humanfs/node) fixed at 20:50:19 UTC. The subsequent repository-owner authenticated `npm run security:github-settings:auth` passed with zero open Dependabot alerts, enabled vulnerability alerts and security updates, and a readable Dependency Graph SBOM containing 277 packages. The merged lockfile resolves qs 6.16.0 and @humanfs/node 0.16.8; the live advisory audit reports zero vulnerabilities. | Keep authenticated hosted alert visibility and the internet-backed advisory audit in merge/release verification; reopen immediately if either surface reports an unresolved alert. | 2026-10-03 |
...
| SEC-P0-005 | High | Mitigated | Repository owner | Live CodeQL, secret-scanning, and dependency alert state must stay visible and triaged before hosted security can be cited as clean release evidence. | On 2026-09-03, after protected merge commit `9737c46ec26ef63277ed6db2333b27449d4cdca1`, main [CI run 33804575536](https://github.com/JeremiahM18/japanese-kanji-anki-builder/actions/runs/33804575536) and [CodeQL run 33804575591](https://github.com/JeremiahM18/japanese-kanji-anki-builder/actions/runs/33804575591) both succeeded. The subsequent repository-owner authenticated `npm run security:github-settings:auth` passed with zero open CodeQL, secret-scanning, and Dependabot alerts. Alerts 6 and 7 are fixed; required branch protection still matches the tracked policy. This closes the dependency-maintenance blocker without asserting a new release or human/device QA. | Keep the hosted audit in merge/release verification and reopen if any required main run fails or any CodeQL, secret-scanning, or Dependabot alert remains unresolved. | 2026-10-03 |
