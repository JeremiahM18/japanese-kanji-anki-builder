...
| SEC-P0-004 | High | Open | Repository owner | Release attestation creation and all-file constrained verification are tracked, but hosted evidence has not yet proven them after a tagged-ref Release workflow. | 2026-08-01 local governance hardening makes the workflow download and validate exact draft APKG inputs, attest every final staged asset, and run constrained verification for every asset before publishing. Owner-authenticated `npm run security:github-settings:auth` must still fail on `artifact_attestation_verification_unproven` until the merged workflow succeeds on a real tag. [github-repository-settings-checklist.md](github-repository-settings-checklist.md), [release-process.md](release-process.md), [../.github/workflows/release.yml](../.github/workflows/release.yml) | Keep the owner decision `Open`; merge through protected main, run the exact tagged-ref Release workflow, download and verify assets, then rerun the owner-authenticated hosted audit before recording mitigation. | 2026-08-07 |
