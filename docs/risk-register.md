...
| SEC-P0-003 | High | Blocked external | Repository owner | Dependabot alert state is not visible to the current GitHub CLI OAuth token, so open dependency alerts cannot be proven absent. | 2026-06-02 authenticated `npm run security:github-settings` returns `403` for the Dependabot alerts endpoint; branch protection, secret scanning, push protection, CodeQL visibility, and private vulnerability reporting are otherwise owner-auth verified. | Refresh owner-auth credentials or use a fine-grained token that can read Dependabot alerts, then rerun `npm run security:github-settings` and update [github-repository-settings-checklist.md](github-repository-settings-checklist.md). | 2026-06-16 |
