...
| SEC-P0-005 | High | Mitigated | Repository owner | Live CodeQL alert state must stay visible and triaged so CodeQL can be used as clean hosted release evidence. | 2026-07-07 owner-authenticated `npm run security:github-settings:auth` verifies latest hosted CI and CodeQL conclusions are `success`, open CodeQL alerts are `0`, open secret-scanning alerts are `0`, and open Dependabot alerts are `0` after PR #184 resolved CodeQL alert #27. | Keep CodeQL, secret-scanning, and Dependabot alert visibility in the hosted audit; reopen if any hosted alert count becomes nonzero without explicit accepted-risk rationale. | 2026-08-07 |
