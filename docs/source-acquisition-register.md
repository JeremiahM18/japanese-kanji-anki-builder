...
| `kanjipedia` | Restricted field-bound use only through current-standard Platinum review notes and `templates/kanji_card_field_source_contract.json`. | Manual kanji-card field verification citations for exact card readings and meanings. | Bulk import, copied entries, JLPT placement truth, generated TSV evidence, or release certification by itself. |
