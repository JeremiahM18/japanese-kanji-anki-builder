...
| --- | --- | --- | --- | --- |
| `@huggingface/transformers@3.8.1` | `sharp@0.35.3` | Outside the parent-declared `^0.34.1` range | Patched resolution for `GHSA-f88m-g3jw-g9cj`; repository use is fail-closed to the one declared `feature-extraction` import and the active embedding model task. | Revalidate the live embedding benchmark and upstream compatibility by `2026-09-28`. |
| `onnxruntime-node@1.21.0` | `tar@7.5.22` | Inside the parent-declared `^7.0.1` range | Pins a resolution newer than the vulnerable `GHSA-r292-9mhp-454m` range. | Revalidate the NLP scope by `2026-09-28`. |
...
- Keep reviewed license exceptions in [../templates/dependency_license_policy.json](../templates/dependency_license_policy.json) current by exact package version, reason, owner, `reviewedAt`, and `nextReview`. The current sharp/libvips exception review is due on 2026-10-03.
