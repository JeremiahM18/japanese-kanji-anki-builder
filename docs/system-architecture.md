...
| Words | 2505 | 500 | Current word Obsidian v2.5 covers the current N5 v2.5 batches. Legacy N5/N4 word proof history remains audit-visible, but it is not current v2.5 certification. |
...
- Word locked scope: `500/1307` current word Obsidian v2.5-certified for current N5/N4 generated rows; `788` Platinum-passing rows need Obsidian v2.5 and `19` rows are blocked on lower lanes. Legacy word proof history remains audit-visible at `1118` N5/N4 targets and `1618` raw ledger events, with `396` older same-target proof events superseded by current v2.5 proof.
- Proof ledger validation: `2600` events across 6 JSONL files.
