...
| Words | 2505 | 400 | Current word Obsidian v2.5 covers the current N5 v2.5 batches. Legacy N5/N4 word proof history remains audit-visible, but it is not current v2.5 certification. |
...
- Word locked scope: `400/1307` current word Obsidian v2.5-certified for current N5/N4 generated rows; `888` Platinum-passing rows need Obsidian v2.5 and `19` rows are blocked on lower lanes. Legacy word proof history remains audit-visible at `1118` N5/N4 targets and `1518` raw ledger events, with `353` legacy events superseded by current v2.5 proof.
- Proof ledger validation: `2500` events across 6 JSONL files.
