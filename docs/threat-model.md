...
| GitHub Actions to published prerelease | exact APKGs, evidence packet, SBOM, dependency-license summary, verification archive, checksum manifest | pinned actions, top-level read-only permissions, verifier job-scoped draft-read access, bundle-job-only publishing/attestation permissions, all-file checksums and attestations |
...
| GitHub workflow requests excessive permission | Token abuse, artifact tampering, or unwanted publishing | top-level read-only permissions; verifier job-scoped `contents: write` required to access the private draft but no publication step; bundle-job-only content/attestation writes; action SHA pinning; supply-chain audit | Hosted branch protection and draft-access permission behavior are drift-prone and must be rechecked before release-trust claims |
| Release inputs or bundle are tampered with or misunderstood | Users trust an unverified APKG, SBOM, packet, or report | exact input membership, packet hashes/counts, APKG inspection, checksum manifest, SBOM, provenance/SBOM attestations, immutable Actions evidence, all-file constrained verification, explicit release-class label | `v0.3.0-beta.5` most recently proved the full hosted path; every future release must independently repeat it and reopen `SEC-P0-004` on drift or failure |
...
| Private vulnerability details are disclosed publicly too early | Exploit details or private data leak | SECURITY.md reporting rules; incident response runbook; minimal public issue guidance; hosted private vulnerability reporting | Hosted intake can drift; keep owner-authenticated verification mandatory and reopen `SEC-P0-002` if it is disabled |
