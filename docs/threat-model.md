...
| Release inputs or bundle are tampered with or misunderstood | Users trust an unverified APKG, SBOM, packet, or report | exact input membership, packet hashes/counts, APKG inspection, checksum manifest, SBOM, provenance/SBOM attestations, immutable Actions evidence, all-file constrained verification, explicit release-class label | `v0.3.0-beta.4` proved the full hosted path; every future release must independently repeat it and reopen `SEC-P0-004` on drift or failure |
