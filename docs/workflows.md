...
Native `.apkg` creation may reuse a SHA-verified, content-addressed generated cache beside the selected package output root when exports, note schema, media integrity, and the APKG builder script are unchanged. The default build therefore uses `out/.apkg-cache`, while isolated run/test roots keep their cache inside the same isolated generated-output parent instead of leaking entries into the repository-global cache. Cache entries include a generated metadata manifest binding the cache key, artifact byte size, and APKG SHA-256. A cache miss rebuilds the package normally; a corrupt cache entry is ignored and rebuilt. The cold native APKG path is independently measurable with `npm run bench:build:cold-apkg:gate`, which clears the default generated APKG cache before the measured build and verifies the Python archive path rather than the hot-cache path.
