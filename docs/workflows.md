...
Use `npm run deck:words:multi-lane-status -- --level=<level>` when the current status of Silver, Gold, Sapphire, Platinum, and Obsidian is needed in one fail-closed process. The command requires explicit level scope, follows certification order, loads/deep-freezes shared generated rows and tracked lane inputs once, and shares read-only lookup indexes only. Silver, Gold, Sapphire, Platinum, and Obsidian still invoke independent evaluators and retain independent results; a prior lane result is never reused as another lane's approval. The command is read-only and does not replace the owning lane commands, write review/proof state, promote cards, shrink denominators, or certify release readiness. Use `npm run bench:word-multi-lane-status -- --level=<level> --repeat=3` for same-machine diagnostic timing evidence; the benchmark has no timing budget and preserves the status command's nonzero result.
