...
Dictionary common-pool editorial queues are ordered by a transparent learner-utility score before the `200`-row cap is applied. The score is an ordering signal only, never card approval. Its components must stay visible for every row: everyday usefulness, concrete/common domain fit, target-kanji reinforcement value, duplicate or near-duplicate safety, specialized/proper-noun penalty signals, exampleability, and pitch/audio/media readiness. Human reviewers still decide keep/defer/reject/move, but the report must print the score, band, reasons, and penalties so a reviewer can see why a row entered the current queue.
