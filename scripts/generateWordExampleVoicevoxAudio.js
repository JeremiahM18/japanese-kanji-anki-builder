...
const { buildWordExampleAudioScope } = require("../src/services/wordAudioService");
const {
...
function parseArgs(argv) {
    ...
    for (const arg of argv) {
        ...
        if (arg.startsWith("--level=")) {
            ...
            throw new Error(`Unknown argument for generateWordExampleVoicevoxAudio: ${arg}`);
        }
    }
...
async function main() {
    ...
    const summary = await generateVoicevoxAudioForWordExampleList({
        words: wordScope,
        config,
    ...
    process.stdout.write(formatWordExampleVoicevoxGenerationSummary(summary, {
        speakerId: resolvedSpeakerId,
