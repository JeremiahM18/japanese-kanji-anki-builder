...
async function main() {
    ...
    assertNoUnknownArgs("deck:words:platinum:certify-status", options.unknownArgs);
    ...
    const summary = buildPlatinumWordCertificationStatusSummary(levelReports);

    if (options.json) {
        process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
    } else {
        process.stdout.write(formatPlatinumWordCertificationStatusReport(summary));
    }
