...
function normalizeSentenceReadingForSynthesis(value) {
    return katakanaToHiragana(String(value || ""))
        .replace(/\s+/g, "")
        .trim();
}
...
function buildAudioSourceSidecarPayload({
    ...
    return {
        ...
        identityHash: normalizeAudioSidecarField(identityHash),
...
async function generateVoicevoxAudioForWordExampleList({
    words,
    config,
    speakerId,
    concurrency,
    ...
    await mapWithConcurrency(words, concurrency || config.exportConcurrency, async (entry) => {
        const outputPath = resolveAudioSourceOutputPath(config.audioSourceDir, buildWordExampleAudioSourceFileName(entry));
        if (!overwrite && fs.existsSync(outputPath)) {
            summary.skippedExisting += 1;
            summary.results.push({
                word: entry.written,
        ...
        try {
            const synthesisReading = normalizeSentenceReadingForSynthesis(entry.exampleReading);
            const readingForValidation = stripSentencePunctuation(synthesisReading);
            if (!synthesisReading || !readingForValidation || !isKanaOnly(readingForValidation)) {
                throw new Error("No kana example-sentence reading available for synthesis.");
            }

            const audioBuffer = await voicevoxClient.synthesize({
                text: synthesisReading,
                speakerId,
            });
            writeManagedVoicevoxAudio(outputPath, audioBuffer);
            writeAudioSourceSidecar({
                ...
                identityHash: entry.identityHash,
                notes: [
                    `Generated by VOICEVOX speaker ${speakerId} for word example ${entry.written}|${entry.reading}`,
                    `host kanji ${entry.hostKanji}`,
                    `identity ${entry.identityHash}`,
                    `synthesized reading ${synthesisReading}`,
                ].join("; "),
            });
...
function formatWordExampleVoicevoxGenerationSummary(summary, options = {}) {
    const lines = [];
    lines.push("Japanese Kanji Builder Word Example Sentence VOICEVOX Audio Generation");
    lines.push("");
    lines.push(`Speaker ID: ${options.speakerId}`);
    lines.push(`Audio destination: ${options.audioSourceDir}`);
    lines.push(`Total word examples: ${summary.totalWords}`);
    lines.push(`Generated: ${summary.generated}`);
    lines.push(`Skipped existing: ${summary.skippedExisting}`);
    lines.push(`Failed: ${summary.failed}`);
    ...
    if (failedRows.length > 0) {
        ...
        lines.push("Failures:");
        for (const row of failedRows.slice(0, 20)) {
            lines.push(`- ${row.word} (${row.reading}): ${row.error}`);
        }
    }
    ...
    if (generatedRows.length > 0) {
        ...
        lines.push("Sample generated:");
        for (const row of generatedRows.slice(0, 10)) {
            lines.push(`- ${row.word} (${row.reading}): ${row.identityHash}`);
        }
    }
...
module.exports = {
    ...
    cleanVoiceLabel,
    formatWordExampleVoicevoxGenerationSummary,
    formatVoicevoxGenerationSummary,
    formatVoicevoxSpeakerTable,
    generateVoicevoxAudioForWordExampleList,
