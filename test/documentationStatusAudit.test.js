...
test("documentation status snapshots match tracked N3 word lane counts", () => {
    ...
    assert.equal(report.snapshot.gold.ratio, "946/1081");
    assert.equal(report.snapshot.gold.missing, 135);
...
test("documentation status audit catches stale N3 word Gold counts", () => {
    ...
    files["README.md"] = files["README.md"].replace("946/1081", "315/1081").replace("`135` generated rows still missing Gold", "`766` generated rows still missing Gold");
    files["CHANGELOG.md"] = files["CHANGELOG.md"].replace("946/1081", "315/1081").replace("`135` generated rows still missing", "`766` generated rows still missing");

    const report = auditDocumentationText({
        files,
        n3WordSnapshot: {
            denominator: 1081,
            gold: { ratio: "946/1081", missing: 135 },
