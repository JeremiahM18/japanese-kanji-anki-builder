...
const HEADER = "Kanji\tDisplayWord\tMeaningJP\tPrimaryReading\tStudyWordKanji\tOnReading\tKunReading\tStrokeOrder\tStrokeOrderImage\tStrokeOrderAnimation\tAudio\tRadical\tNotes\tExampleSentence";
...
test("buildKanjiDeckContractReport accepts individual-kanji card anchors", () => {
    const report = buildKanjiDeckContractReport({
        ...
        tsv: [
            HEADER,
            "車\t車\t車 （くるま） ／ car\tくるま\t\tオン: シャ\tくん: くるま\t<img src=\"8ECA.gif\" />\t<img src=\"8ECA.png\" />\t<img src=\"8ECA.gif\" />\t[sound:8ECA_車-kanji-reading-車-くるま.wav]\t車\t電車 （でんしゃ） - train\t電車で行きます。",
        ].join("\n"),
    });
...
test("buildKanjiDeckContractReport rejects compound kanji-deck anchors", () => {
    const report = buildKanjiDeckContractReport({
        ...
        tsv: [
            HEADER,
            "車\t電車\t電車 （でんしゃ） ／ train\tでんしゃ\t車: JLPT N5\tオン: シャ\tくん: くるま\t\t\t\t[sound:8ECA_車-kanji-reading-電車-でんしゃ.wav]\t車\ttrain context\t電車で行きます。",
        ].join("\n"),
    });
...
test("buildKanjiDeckContractReport rejects missing primary readings product-wide", () => {
    const report = buildKanjiDeckContractReport({
        ...
        tsv: [
            HEADER,
            "誕\t誕\t誕 ／ birth\t\t\tオン: タン\t\t\t\t\t\t言\t誕生日 （たんじょうび） - birthday\t誕生日を祝います。",
        ].join("\n"),
    });
