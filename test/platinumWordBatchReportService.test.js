...
test("word batch sentence evidence allows normal inflected readings", () => {
    ...
    assert.equal(exampleSentenceContainsWrittenWord("映画を静かに観ました。", "観る"), true);
    assert.equal(exampleSentenceContainsWrittenWord("多くの経験を経て成長しました。", "経る"), true);
    assert.equal(exampleSentenceContainsWrittenWord("火を絶やさないように注意します。", "絶やす"), true);
    assert.equal(exampleSentenceContainsWrittenWord("判断を過たないように注意しました。", "過つ"), true);
    assert.equal(exampleSentenceContainsWrittenWord("うわさを散蒔かないでください。", "散蒔く"), true);
    assert.equal(exampleSentenceContainsWrittenWord("用事を午前中に済ませました。", "済ます"), true);
    ...
    assert.equal(exampleReadingContainsWordReading("えいがをしずかにみました", "みる"), true);
    assert.equal(exampleReadingContainsWordReading("おおくのけいけんをへてせいちょうしました", "へる"), true);
    assert.equal(exampleReadingContainsWordReading("ひをたやさないようにちゅういします", "たやす"), true);
    assert.equal(exampleReadingContainsWordReading("はんだんをあやまたないようにちゅういしました", "あやまつ"), true);
    assert.equal(exampleReadingContainsWordReading("うわさをばらまかないでください", "ばらまく"), true);
    assert.equal(exampleReadingContainsWordReading("ようじをごぜんちゅうにすませました", "すます"), true);
