...
test("CI workflow uses tracked-input governance checks and documents local-data gates", () => {
    ...
    assert.match(readme, /Clean CI runs `data:obsidian:proof:validate`, `data:obsidian:proof:reconcile -- --levels=5,4,3`, `data:obsidian:proof:provider-parity -- --levels=5,4,3 --row-source=tracked-review-set`/);
