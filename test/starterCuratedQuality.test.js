...
test("resolved curated N3-N5 entries keep selected learner-facing editorial choices stable", () => {
    ...
    assert.deepEqual(curatedStudyData["客"].displayWord, { written: "客", pron: "きゃく" });
    assert.deepEqual(curatedStudyData["客"].preferredWords, ["客", "お客さん", "乗客"]);

    assert.deepEqual(curatedStudyData["内"].displayWord, { written: "国内", pron: "こくない" });
    assert.deepEqual(curatedStudyData["内"].preferredWords, ["国内", "以内", "内"]);
...
test("resolved tracked N1 batch entries keep selected learner-facing editorial choices stable", () => {
    ...
    assert.deepEqual(curatedStudyData["只"].blockedMeanings, ["in addition"]);
    assert.equal(curatedStudyData["只"].exampleSentence.reading, "ただいまたんとうしゃがまいりますので、しょうしょうおまちください。");
    assert.deepEqual(curatedStudyData["叶"].blockedMeanings, ["answer"]);
    assert.deepEqual(curatedStudyData["吏"].blockedMeanings, ["an official"]);
    assert.deepEqual(curatedStudyData["吐"].blockedMeanings, ["belch", "tell (lies)"]);
    assert.deepEqual(curatedStudyData["喝"].blockedMeanings, ["hoarse"]);
    assert.deepEqual(curatedStudyData["嘱"].blockedMeanings, ["send a message"]);
    assert.deepEqual(curatedStudyData["噴"].blockedMeanings, ["flush out"]);
    assert.deepEqual(curatedStudyData["器"].blockedMeanings, ["set"]);
    assert.deepEqual(curatedStudyData["嚇"].blockedMeanings, ["dignity", "majesty"]);
    assert.deepEqual(curatedStudyData["囚"].blockedMeanings, ["catch", "criminal"]);
    assert.deepEqual(curatedStudyData["圏"].blockedMeanings, ["radius"]);
    assert.deepEqual(curatedStudyData["圭"].blockedMeanings, ["square jewel"]);
    assert.deepEqual(curatedStudyData["坪"].blockedMeanings, ["two-mat area"]);
    assert.deepEqual(curatedStudyData["垂"].blockedMeanings, ["slouch"]);
    assert.equal(curatedStudyData["執"].englishMeaning, "take in hand / administer / persist");
    assert.deepEqual(curatedStudyData["執"].blockedMeanings, ["devote oneself", "take to heart", "tenacious"]);
    assert.deepEqual(curatedStudyData["基"].blockedMeanings, ["counter for machines"]);
    assert.deepEqual(curatedStudyData["堅"].blockedMeanings, ["strict", "tight"]);
    assert.deepEqual(curatedStudyData["堪"].blockedMeanings, ["support"]);
    assert.deepEqual(curatedStudyData["塀"].blockedMeanings, ["(kokuji)"]);
    assert.deepEqual(curatedStudyData["塁"].blockedMeanings, ["base(ball)", "bases", "walls"]);
    assert.deepEqual(curatedStudyData["墜"].blockedMeanings, ["fall (down)"]);
    assert.deepEqual(curatedStudyData["墾"].blockedMeanings, ["ground-breaking"]);
    assert.deepEqual(curatedStudyData["壁"].blockedMeanings, ["lining (stomach)"]);
    assert.deepEqual(curatedStudyData["壇"].blockedMeanings, ["terrace"]);
    assert.deepEqual(curatedStudyData["壊"].blockedMeanings, ["demolition"]);
    assert.deepEqual(curatedStudyData["壮"].blockedMeanings, ["manhood"]);
    assert.deepEqual(curatedStudyData["奇"].blockedMeanings, ["curiosity"]);
    assert.equal(curatedStudyData["奇"].exampleSentence.english, "The story had a somewhat strange atmosphere.");
    assert.equal(curatedStudyData["奈"].englishMeaning, "phonetic name element");
    assert.deepEqual(curatedStudyData["奈"].blockedMeanings, ["Nara"]);
    assert.equal(curatedStudyData["奉"].englishMeaning, "offer / dedicate / serve");
    assert.deepEqual(curatedStudyData["奉"].blockedMeanings, ["observance", "present"]);
    assert.deepEqual(curatedStudyData["奔"].blockedMeanings, ["bustle"]);
    assert.equal(curatedStudyData["奎"].englishMeaning, "star name / literary constellation");
    assert.deepEqual(curatedStudyData["奎"].blockedMeanings, ["god of literature", "literary brilliance", "star"]);
    assert.deepEqual(curatedStudyData["奮"].blockedMeanings, ["flourish", "strive"]);
    assert.equal(curatedStudyData["奮"].exampleSentence.japanese, "試合の前に少し興奮していました。");
    assert.equal(curatedStudyData["奮"].exampleSentence.reading, "しあいのまえにすこしこうふんしていました。");
    assert.deepEqual(curatedStudyData["如"].blockedMeanings, ["best", "better"]);
    assert.deepEqual(curatedStudyData["如"].displayWord, { written: "如し", pron: "ごとし" });
    assert.ok(curatedStudyData["如"].notes.includes("如し"));
    assert.deepEqual(curatedStudyData["嫌"].displayWord, { written: "嫌う", pron: "きらう" });
    assert.deepEqual(curatedStudyData["嫌"].blockedMeanings, ["unpleasant"]);
    assert.equal(curatedStudyData["嫌"].exampleSentence.japanese, "彼女は人をだます行為を嫌います。");
    assert.equal(curatedStudyData["嫌"].exampleSentence.reading, "かのじょはひとをだますこういをきらいます。");
    assert.deepEqual(curatedStudyData["嫡"].blockedMeanings, ["direct descent (non-bastard)"]);
    assert.deepEqual(curatedStudyData["宙"].blockedMeanings, ["interval of time", "memorization", "mid-air"]);
    assert.deepEqual(curatedStudyData["宜"].blockedMeanings, ["best regards"]);
    assert.equal(curatedStudyData["宥"].englishMeaning, "forgive / soothe");
    assert.deepEqual(curatedStudyData["宥"].blockedMeanings, ["permit"]);
    assert.deepEqual(curatedStudyData["宮"].blockedMeanings, ["constellations", "princess"]);
    assert.deepEqual(curatedStudyData["宵"].blockedMeanings, ["wee hours"]);
    assert.deepEqual(curatedStudyData["寂"].blockedMeanings, ["mature", "mellow", "quietly"]);
    assert.deepEqual(curatedStudyData["密"].blockedMeanings, ["carefulness", "density (pop)", "minuteness"]);
    assert.deepEqual(curatedStudyData["寛"].blockedMeanings, ["be at ease", "broadminded", "feel at home", "relax"]);
    assert.deepEqual(curatedStudyData["妃"].blockedMeanings, ["princess"]);
    assert.deepEqual(curatedStudyData["妄"].blockedMeanings, ["without authority"]);
    assert.deepEqual(curatedStudyData["妙"].blockedMeanings, ["miracle", "mystery", "queer"]);
    assert.deepEqual(curatedStudyData["妥"].blockedMeanings, ["depravity"]);
    assert.deepEqual(curatedStudyData["呉"].displayWord, { written: "呉音", pron: "ごおん" });
    assert.deepEqual(curatedStudyData["呉"].blockedMeanings, ["bestow", "do something for", "give"]);
    assert.equal(curatedStudyData["呉"].exampleSentence.japanese, "呉音は漢字音の一つとして学ばれています。");
    assert.deepEqual(curatedStudyData["哀"].blockedMeanings, ["pathetic", "pathos"]);
    assert.deepEqual(curatedStudyData["哉"].blockedMeanings, ["exclamation mark", "question mark"]);
    assert.deepEqual(curatedStudyData["哲"].displayWord, { written: "哲", pron: "てつ" });
    assert.equal(curatedStudyData["哲"].englishMeaning, "wise / discerning");
    assert.deepEqual(curatedStudyData["哲"].blockedMeanings, ["philosophy"]);
    assert.equal(curatedStudyData["哲"].exampleSentence.japanese, "彼の明哲な判断で混乱を避けられました。");
    assert.deepEqual(curatedStudyData["唄"].preferredWords, ["小唄", "唄う", "長唄"]);
    assert.ok(!curatedStudyData["唄"].notes.includes("民謡"));
    assert.deepEqual(curatedStudyData["唆"].blockedMeanings, ["promote", "seduce"]);
    assert.deepEqual(curatedStudyData["唱"].blockedMeanings, ["call upon", "yell"]);
