...
test("tracked starter word data includes the twenty-seventh governed N4 completion batch", () => {
    ...
    assert.equal(starterEntries["千代|ちよ"].coverage.coversReadings["代"], "よ");
    assert.equal(starterEntries["病み付き|やみつき"].coverage.coversReadings["病"], "やみ");
    assert.equal(starterEntries["白銀|しろがね"].coverage.coversReadings["銀"], "しろがね");
    assert.equal(starterEntries["足りる|たりる"].coverage.coversReadings["足"], "たりる");
    assert.equal(starterEntries["注ぐ|そそぐ"].coverage.coversReadings["注"], "そそぐ");
    assert.equal(starterEntries["通す|とおす"].coverage.coversReadings["通"], "とおす");
    assert.equal(starterEntries["動かす|うごかす"].coverage.coversReadings["動"], "うごかす");
    assert.equal(starterEntries["売れる|うれる"].coverage.coversReadings["売"], "うれる");
    assert.equal(starterEntries["歩む|あゆむ"].coverage.coversReadings["歩"], "あゆむ");
    assert.equal(starterEntries["着せる|きせる"].coverage.coversReadings["着"], "きせる");
    assert.equal(starterEntries["着ける|つける"].coverage.coversReadings["着"], "つける");
    assert.equal(starterEntries["代える|かえる"].coverage.coversReadings["代"], "かえる");
    assert.equal(starterEntries["代わる|かわる"].coverage.coversReadings["代"], "かわる");
    assert.equal(starterEntries["身代わり|みがわり"].coverage.coversReadings["代"], "がわり");
    assert.equal(starterEntries["転がる|ころがる"].coverage.coversReadings["転"], "ころがる");
    assert.equal(starterEntries["転がす|ころがす"].coverage.coversReadings["転"], "ころがす");
    assert.equal(starterEntries["病み付き|やみつき"].readingBreakdown, "<ruby>病<rt>や</rt></ruby>み<ruby>付<rt>つ</rt></ruby>き");
});
...
test("tracked starter word data includes the twenty-eighth governed N4 completion batch", () => {
    ...
    assert.equal(starterEntries["愛猫|あいびょう"].coverage.coversReadings["猫"], "びょう");
    assert.equal(starterEntries["主人|あるじ"].coverage.coversReadings["主"], "あるじ");
    assert.equal(starterEntries["病|やまい"].coverage.coversReadings["病"], "やまい");
    assert.equal(starterEntries["会わせる|あわせる"].coverage.coversReadings["会"], "あわせる");
    assert.equal(starterEntries["開ける|ひらける"].coverage.coversReadings["開"], "ひらける");
    assert.equal(starterEntries["究める|きわめる"].coverage.coversReadings["究"], "きわめる");
    assert.equal(starterEntries["計らう|はからう"].coverage.coversReadings["計"], "はからう");
...
test("tracked starter word data includes the twenty-ninth governed N4 completion batch", () => {
    ...
    assert.equal(starterEntries["京阪|けいはん"].coverage.coversReadings["京"], "けい");
    assert.equal(starterEntries["緑青|ろくしょう"].coverage.coversReadings["青"], "しょう");
    assert.equal(starterEntries["験担ぎ|げんかつぎ"].coverage.coversReadings["験"], "げん");
    assert.equal(starterEntries["店子|たなこ"].coverage.coversReadings["店"], "たな");
    assert.equal(starterEntries["仮字|かな"].coverage.coversReadings["字"], "な");
    assert.equal(starterEntries["相部屋|あいべや"].coverage.coversReadings["部"], "べ");
    assert.equal(starterEntries["創世記|そうせいき"].coverage.coversReadings["世"], "そう");
    assert.equal(starterEntries["大社|おおやしろ"].coverage.coversReadings["社"], "やしろ");
    assert.equal(starterEntries["転寝|うたたね"].coverage.coversReadings["転"], "うたた");
    assert.equal(starterEntries["裏切り|うらぎり"].coverage.coversReadings["切"], "ぎり");
    assert.equal(starterEntries["夜通し|よどおし"].coverage.coversReadings["通"], "どおし");
    assert.equal(starterEntries["赤銅|しゃくどう"].coverage.coversReadings["赤"], "しゃく");
    assert.equal(starterEntries["夜通し|よどおし"].readingBreakdown, "<ruby>夜<rt>よ</rt></ruby><ruby>通<rt>どお</rt></ruby>し");
});
